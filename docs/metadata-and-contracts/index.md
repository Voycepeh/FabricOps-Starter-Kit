...
The steward dropdown reads active rows from `METADATA_DATA_STEWARD`. `00_env_config` creates or checks that table and `METADATA_DATA_AGREEMENT` during environment bootstrap, reports whether active steward rows exist, and never seeds fake people. Teams must populate real steward rows with `is_active = true` before rendering `01_da_*`. Widget defaults are exposed through `CONFIG.data_agreement_config`. The `01_da_*` form notebook calls `render_agreement_intake_app(...)`, which handles commit-button wiring and commit flow inside the framework. Advanced users may call `create_agreement_form(...)`, `read_agreement_form(...)`, `collect_agreement_metadata(...)`, and `commit_agreement_metadata(...)` directly when customizing the workflow. Reads and writes route through `CONFIG.path_config.paths[env]["metadata"]`, so no default attached lakehouse is required.
