...
A `03_pc_*` notebook loads approved rules during pipeline runs, applies them to the current dataframe or table, sends passing rows downstream, and quarantines failed rows with reasons. These approved rules are one enforceable part of the wider [operating model](fabricops-operating-model.md): the shared metadata flow used by governance and engineering.
...
Next read: [Operating Model](fabricops-operating-model.md), [Quick Start](quick-start.md), [Function Reference](reference/index.md).
