# _prepare_dq_profile_input_rows

**Module:** `data_quality`  
**Classification:** Internal

## Purpose

No summary available.

## Function manifest

- Fully qualified function name: `fabricops_kit.data_quality._prepare_dq_profile_input_rows`
- Short name: `_prepare_dq_profile_input_rows`
- Module: `data_quality`
- Classification: Internal
- Related module: `data_quality`
- Source file path: `src/fabricops_kit/data_quality.py`
- Source reference: <a href="../../api/modules/data_quality/#_prepare_dq_profile_input_rows">Module source anchor</a>
- Inbound references count: 1
- Outbound references count: 1

## Inbound references
- <a href="../internal/data_quality/_draft_dq_rules/"><code>fabricops_kit.data_quality._draft_dq_rules</code></a>

## Outbound references
- <a href="../profile_dataframe/"><code>fabricops_kit.data_profiling.profile_dataframe</code></a>
