...
`03_incremental_pipeline` is the separate **target-aware incremental pipeline template**. Each target is defined before its sources so `pipeline_read()` can resolve the watermark or changed partitions last committed for that exact source → target relationship. A first run bootstraps with the complete source; incremental append permits that bootstrap only for a new or empty target, while SCD1 and SCD2 retain their keyed idempotent merge semantics. Later no-new-data runs can skip publication explicitly. Removed partitions are actionable work and can clear stale target rows only through a matching governed partition-scoped overwrite. The starter flow mixes one incremental driving source with full supporting sources, applies Source Drift to every source, and profiles only complete full sources or complete persisted targets. When one notebook publishes multiple targets, every target write is an independent publication boundary—there is no cross-target atomicity.
