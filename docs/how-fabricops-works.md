...
The FabricOps overview video introduces the story. This page explains the **end-to-end operating model**: how Governance and Engineering work together from contract authoring through validation, activation, promotion, and Production use.
...
5. **Governance activates the tested definition.** [`widget_data_contract()`](api/reference/widget_data_contract.md) links the exact Data Agreement version and tags one frozen Data Contract version as active for that `table_id`. Frozen versions stay frozen; activation does not change their governed JSON. Development can validate any frozen version. Production is strict: it must resolve exactly one frozen version with `is_active=true` for each linked `table_id`.
