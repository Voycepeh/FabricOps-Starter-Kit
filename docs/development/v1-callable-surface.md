...
27. `widget_select_governance_profile_target`
...
Non-v1 helpers that are still needed internally now use leading-underscore names in source modules. Examples include `_widget_review_table_governance`, `_build_runtime_audit_fields`, `_get_governance_metadata_schemas`, `_build_column_context_records`, `_commit_dq_rules`, `_print_runtime_banner`, and `_validate_dq_rules`.

## 03_governance wrapper rationale

`get_selected_catalogue_table` remains public because users must explicitly read the stable table identity after interacting with `widget_select_catalogue_table` or `widget_select_governance_profile_target`. It accepts the selector returned by the widget for readability while preserving widget state fallback.
