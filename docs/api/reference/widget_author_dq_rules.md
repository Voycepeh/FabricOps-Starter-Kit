...
??? info "Call flow"
    ...
    **Call chains deeper than 4 levels**
    ...
    - `widget_author_dq_rules` → `_dq_records_from_selection` → `_base_guardrail_rule_record` → `_now_utc_iso` → `_current_audit_timestamp` → `_get_audit_timezone`
    - `widget_author_dq_rules` → `_dq_records_from_selection` → `_base_guardrail_rule_record` → `_now_utc_iso` → `_current_audit_timestamp` → `_get_audit_timezone` → `_validate_audit_timezone`
    - `widget_author_dq_rules` → `_dq_records_from_selection` → `_base_guardrail_rule_record` → `_resolve_action_by` → `_runtime_context` → `_context_get`
    - `widget_author_dq_rules` → `_dq_records_from_selection` → `_base_guardrail_rule_record` → `guardrail_authoring_status` → `_authoring_lifecycle` → `_is_no_approval_required`
    - `widget_author_dq_rules` → `_dq_records_from_selection` → `_base_guardrail_rule_record` → `guardrail_authoring_status` → `_authoring_lifecycle` → `_lifecycle_fields`
    - `widget_author_dq_rules` → `_dq_records_from_selection` → `_base_guardrail_rule_record` → `guardrail_authoring_status` → `_authoring_lifecycle` → `_now_utc_iso`
    - `widget_author_dq_rules` → `_dq_records_from_selection` → `_base_guardrail_rule_record` → `guardrail_authoring_status` → `_authoring_lifecycle` → `_now_utc_iso` → `_current_audit_timestamp`
    - `widget_author_dq_rules` → `_dq_records_from_selection` → `_base_guardrail_rule_record` → `guardrail_authoring_status` → `_authoring_lifecycle` → `_now_utc_iso` → `_current_audit_timestamp` → `_get_audit_timezone`
    ...
    **Helpers that only call one package-local helper**
    ...
    - `_dq_records_from_selection` only delegates to `_base_guardrail_rule_record`.
