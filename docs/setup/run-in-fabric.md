...
- Run `00_env_config` and confirm metadata table validation passes. The active setup registry contains the current 10 standard Lakehouse Delta metadata tables. `METADATA_DATA_ACCESS` is optional manual/offline governance metadata and is not created by the standard active setup registry. Metadata setup writes missing tables to the configured `metadata` lakehouse target and does not require a default lakehouse attachment.
