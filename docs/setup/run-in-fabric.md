...
- Run `00_env_config` and confirm metadata table validation passes. The active setup registry contains the current 11 Lakehouse Delta metadata tables, including `METADATA_DATA_ACCESS` as access-capture context metadata. Metadata setup writes missing tables to the configured `metadata` lakehouse target and does not require a default lakehouse attachment.
