(function () {
  ...
  async function initCallGraph() {
    ...
    const nodeRecords = []; const edgeRecords = []; const inbound = new Map(); const outbound = new Map(); const byModule = new Map();
    ...
    for (const [id, c] of entries) for (const target of (Array.isArray(c.calls) ? c.calls : [])) if (knownNodes.has(target)) { edgeRecords.push({ id: `${id}->${target}`, from: id, to: target }); outbound.get(id).add(target); inbound.get(target).add(id); }

    canvas.innerHTML = '';
    const toolbar = document.createElement('div'); toolbar.className = 'call-graph-view-toggle'; toolbar.innerHTML = '<button type="button" data-mode="function">Focused view</button><button type="button" data-mode="full">Full map</button>'; canvas.appendChild(toolbar);
    ...
    function refreshEdges() {
      ...
      for (const edge of edgeRecords) {
        const fromChip = functionToChip.get(edge.from); const toChip = functionToChip.get(edge.to); if (!fromChip || !toChip) continue;
    ...
    toolbar.addEventListener('click', (e) => {
      ...
      if (mode === 'full') { selectedModuleOnly = null; setMode('full'); updateMapVisibility(); focusedLayout.innerHTML=''; }
      if (mode === 'function') {
    ...
    if (functionQuery) { submitSearch(functionQuery); return; }
    if (moduleQuery) { selectedModuleOnly = moduleQuery.replace(/^fabricops_kit\./, ''); setMode('module'); updateMapVisibility(); renderFocusedFunction(null); }
    else { setMode('full'); updateMapVisibility(); }
    refreshEdges();
  }
