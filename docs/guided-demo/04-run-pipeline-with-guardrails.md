...
**Return to the same `02_pipeline` in Engineering Development, select the frozen Data Contract version for each governed `table_id`, and validate it against the real pipeline.**
...
4. Rerun the same visible **Read → Transform → Write** path used in Step 2. The selected frozen version now supplies the Guardrails evaluated around that path.
5. Evaluate the saved immutable requirements with the modular runtime functions: `check_schema()`, `check_freshness()`, `check_source_drift()`, `check_dq()`, and `check_sensitive_data()` where applicable.
