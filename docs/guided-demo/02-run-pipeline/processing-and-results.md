...
## Keep Source Drift separate from processing

Freshness asks whether the source is recent enough. Source Drift asks whether data already consumed by this target changed unexpectedly. For example, today's arrival can pass Freshness while a value consumed yesterday changing from `$20` to `$25` is still detected as historical mutation. The source table's governed load strategy defines compatibility: new data is compatible with append, while changed, removed, or reappeared history violates an append source rule. The target identity selects the correct last-successful baseline but its own write strategy is irrelevant to the drift rule. Source Drift reports the evidence without selecting or executing target processing.
