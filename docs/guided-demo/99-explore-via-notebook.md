...
A Fabric notebook can attach one Lakehouse or Warehouse for convenient native access. When data lives in another configured item, FabricOps IO provides a consistent way to resolve the Production target without switching attachments or hardcoding paths throughout the notebook.
...
| `profile_dataframe` | Profile a Spark DataFrame returned from a Production read. |
...
The consumer team can read Engineering Production data through configured FabricOps targets, inspect it as a Spark DataFrame, and generate project-level profiles where useful.
