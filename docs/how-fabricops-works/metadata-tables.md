...
| profile_stage | source | Runtime collected | Distinguishes source and target profile evidence for preset baseline lookup. |
...
| source_data_change_check | changing_data | Runtime collected | Source data-change preset configured in `03_pc`: `changing_data`, `fixed_data`, `monitor_changing_data`, or `monitor_fixed_data`. |
