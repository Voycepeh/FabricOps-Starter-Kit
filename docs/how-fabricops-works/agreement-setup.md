...
| --- | --- | --- |
| Data steward intake | [`widget_render_data_steward`](../api/reference/widget_render_data_steward.md) | [`METADATA_DATA_STEWARD`](metadata-tables.md#metadata_data_steward) |
| Agreement intake | [`widget_render_data_agreement`](../api/reference/widget_render_data_agreement.md) | [`METADATA_DATA_AGREEMENT`](metadata-tables.md#metadata_data_agreement) |
| Agreement evidence | [`widget_render_agreement_evidence`](../api/reference/widget_render_agreement_evidence.md) | [`METADATA_DATA_AGREEMENT_EVIDENCE`](metadata-tables.md#metadata_data_agreement_evidence) |

Use this page as the notebook-facing guide. Use the [metadata tables reference](metadata-tables.md) for column-level table details and the [Environment Configuration](environment-config.md#agreement-widget-configuration) page for the source of truth for widget configuration.
...
| Metadata written | One active or historical steward row in [`METADATA_DATA_STEWARD`](metadata-tables.md#metadata_data_steward), including generated `steward_id`, lifecycle fields, `custom_fields_json`, and runtime audit columns. |
| Metadata table documentation | [`METADATA_DATA_STEWARD`](metadata-tables.md#metadata_data_steward) |

!!! info "Related metadata"
    Data steward intake writes [`METADATA_DATA_STEWARD`](metadata-tables.md#metadata_data_steward). Agreement intake and agreement selection read those steward rows so later notebook evidence can point to an accountable steward.
...
| Metadata written | One agreement row in [`METADATA_DATA_AGREEMENT`](metadata-tables.md#metadata_data_agreement), including generated `agreement_id`, `contract_version`, selected steward context, `custom_fields_json`, and runtime audit columns. |
| Metadata table documentation | [`METADATA_DATA_AGREEMENT`](metadata-tables.md#metadata_data_agreement) |

!!! info "Related metadata"
    Agreement intake writes [`METADATA_DATA_AGREEMENT`](metadata-tables.md#metadata_data_agreement). `02_pipeline` later reads this table through the agreement selector so pipeline summary, lineage, and guardrail evidence can be tied back to the selected agreement.
...
| Metadata written | One evidence reference row per saved file reference in [`METADATA_DATA_AGREEMENT_EVIDENCE`](metadata-tables.md#metadata_data_agreement_evidence). The widget writes metadata rows only; it does not write binary file content. |
| Metadata table documentation | [`METADATA_DATA_AGREEMENT_EVIDENCE`](metadata-tables.md#metadata_data_agreement_evidence) |

!!! info "Related metadata"
    Agreement evidence writes [`METADATA_DATA_AGREEMENT_EVIDENCE`](metadata-tables.md#metadata_data_agreement_evidence). Dashboard and handover views can use those rows to explain which files or references support the selected agreement.
...
- accountable steward context to [`METADATA_DATA_STEWARD`](metadata-tables.md#metadata_data_steward);
- agreement identity, scope, usage, and steward linkage to [`METADATA_DATA_AGREEMENT`](metadata-tables.md#metadata_data_agreement);
- supporting file-reference evidence to [`METADATA_DATA_AGREEMENT_EVIDENCE`](metadata-tables.md#metadata_data_agreement_evidence).
