...
`widget_select_agreement` is the downstream runtime entry point because agreement selection anchors `02_pipeline` to an approved data agreement. The widget can register the active notebook in `METADATA_NOTEBOOK_REGISTRY`, preserving `agreement_id`, `contract_version`, `notebook_registry_id`, and `notebook_id` for pipeline metadata, lineage, and run-summary evidence.
