...
def replace_once(text: str, pattern: str, replacement: str, label: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"Could not replace {label} exactly once; found {count} matches")
    return updated


def main() -> None:
    metadata_path = Path("scripts/reference_docs_metadata.py")
    metadata = metadata_path.read_text(encoding="utf-8")
    metadata = replace_once(
        metadata,
        r"METADATA_TABLE_PURPOSES = \{.*?\n\}\n\n(?=_UNTRACED_SCHEMA_OWNER = \{)",
        METADATA_MODELS_BLOCK + "\n\n",
        "metadata purpose contract",
    )
    metadata_path.write_text(metadata, encoding="utf-8")

    generator_path = Path("scripts/generate_individual_function_reference_pages.py")
    generator = generator_path.read_text(encoding="utf-8")
    generator = replace_once(
        generator,
        r"def generate_metadata_reference_pages\(\) -> None:\n.*?\n\ndef _trim_callable_flow_dashboard_contract\(",
        GENERATOR_FUNCTION + "\n\n\ndef _trim_callable_flow_dashboard_contract(",
        "metadata generator function",
    )
    generator_path.write_text(generator, encoding="utf-8")

    Path("tests/unit/test_metadata_reference_generator.py").write_text(TEST_FILE, encoding="utf-8")
