...
MODULE_DOCS_METADATA: list[ModuleDocMetadata] = [
    ...
    {"module_name": "governance_review", "visibility": "public", "module_summary": "Owns table-scoped 03_review catalogue selection, business context review, DQ-rule review guidance, classification review, AI-assisted internal drafting helpers, and approved metadata commit through record_table_governance and review_pipeline_run.", "sidebar_group": "1. Governance steward", "sidebar_include": True},
...
TEMPLATE_FLOW_DOCS: list[TemplateFlowDocMetadata] = [{'notebook_key': '00_env_config',
 ...
 {'notebook_key': '03_review',
  ...
  'segments': [{'symbols': ['widget_select_catalogue_table',
                            ...
                            'record_table_governance', 'review_pipeline_run'],
...
PUBLIC_SYMBOL_DOCS: list[PublicSymbolDocMetadata] = [{'kind': 'function',
 ...
 {'kind': 'function',
  'module': 'governance_review',
  'function_type': 'callable',
  'summary_override': 'Review latest or selected pipeline evidence from metadata and write a final governance outcome.',
  'symbol_name': 'review_pipeline_run',
  'template_notebook': '03_review',
  'template_segment': 'Governance review',
  'use_when': 'Use in 03_review after 02_pipeline has written metadata evidence and a reviewer needs an approved, rejected, or needs_remediation outcome.',
  'do_not_use_when': 'Do not use before 02_pipeline evidence exists, as a replacement for agreement intake, or to make pipeline writes.',
  'parameters': 'config, env, spark_session, dataset_name, target_table, optional run_id, agreement_id, reviewed_by, and mode.',
  'returns': 'Governance review row with outcome and evidence status fields.',
  'raises': 'Raises metadata read/write errors when configured metadata tables are unavailable.',
  'side_effects': 'Reads agreement, profile, lineage, DQ, and pipeline-run metadata; appends one row to METADATA_GOVERNANCE_REVIEW.',
  'fabric_context': 'Requires 00_env_config metadata routing and a separate 03_review session with access to the metadata lakehouse.',
  'ai_verification': 'Confirm the outcome and issues_json reflect missing agreement evidence, failed DQ, and schema drift before treating the review as complete.',
  'preferred_example': 'review = review_pipeline_run(CONFIG, ENV, spark_session=spark, dataset_name=DATASET_NAME, target_table=TARGET_TABLE)',
  'related_functions': ['record_table_governance', 'setup_metadata_tables', 'enforce_dq_rules']},]
