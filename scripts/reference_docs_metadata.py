...
PUBLIC_SYMBOL_DOCS: list[PublicSymbolDocMetadata] = [{'kind': 'function',
 ...
 {'kind': 'function',
  ...
  'use_when': 'Use after SOURCE_TABLES or TARGET_TABLES and their defaults are defined to derive standard config fields, load source DataFrames, or add target audit columns.',
  'do_not_use_when': 'Do not use for ad hoc reads or writes outside the pipeline table-config pattern.',
  'parameters': 'table_configs, default_settings, table_role, and role-specific context such as config/env/spark_session for sources or run_id/pipeline_name for targets.',
  ...
  'fabric_context': 'Uses CONFIG and env from 00_env_config for source reads; target audit columns require a Spark-compatible DataFrame.',
  ...
  'preferred_example': 'SOURCE_TABLES, SOURCE_CONFIG_BY_KEY = prepare_pipeline_table_configs(SOURCE_TABLES, DEFAULT_SOURCE_GUARDRAILS, table_role="source", config=CONFIG, env=ENV_NAME, spark_session=spark)',
