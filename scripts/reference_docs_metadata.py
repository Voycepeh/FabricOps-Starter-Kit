...
MODULE_DOCS_METADATA: list[ModuleDocMetadata] = [
    ...
    {"module_name": "pipeline", "visibility": "public", "module_summary": "Owns thin 02_pipeline orchestration helpers for many-source reads, profiles, guardrails, catalogue evidence, target writes, lineage, and runtime summaries.", "sidebar_group": "3. Data engineer", "sidebar_include": True},
...
TEMPLATE_FLOW_DOCS: list[TemplateFlowDocMetadata] = [{'notebook_key': '00_env_config',
 ...
 {'notebook_key': '02_pipeline',
  ...
  'segment_intro': 'Thin production orchestration for many-source, many-target pipeline guardrails, evidence, lineage, and publishing.',
...
PUBLIC_SYMBOL_DOCS: list[PublicSymbolDocMetadata] = [{'kind': 'function',
 ...
 {'kind': 'function',
  ...
  'use_when': 'Use after setup_notebook in 00_env_config to create or validate the FabricOps '
              'metadata tables required by agreement, profiling, lineage, drift, and governance '
              'workflows.',
  ...
  'ai_verification': 'Verify metadata setup completes before recommending agreement, profiling, '
                     'lineage, drift, or governance workflows that persist evidence.',
 ...
 {'kind': 'function',
  ...
  'related_functions': ['monitor_data_changes', 'record_table_governance']},
 {'kind': 'function',
  'module': 'drift',
  'function_type': 'callable',
  ...
  'related_functions': ['monitor_data_changes', 'stop_if_failed']},
 {'kind': 'function',
  'module': 'drift',
  'function_type': 'callable',
  'summary_override': 'Profile data, compare against the approved baseline, and return a drift '
                      'guardrail result.',
  ...
  'parameters': 'spark, dataframe, metadata_table, dataset_name, table_name, required stage, '
                'preset, optional exclude_run_id, distribution columns, and policy overrides.',
  'returns': 'Guardrail result dictionary with status, can_continue, message, current profile, '
             'baseline details, and drift checks.',
  ...
  'preferred_example': 'drift_result = monitor_data_changes(spark, df, "METADATA_PROFILE", '
 ...
 {'kind': 'function',
  'module': 'drift',
  'function_type': 'callable',
  'summary_override': 'Stop a notebook only when a schema or data-change guardrail result blocks '
                      'continuation.',
  'symbol_name': 'stop_if_failed',
  'template_notebook': '02_pipeline',
  'template_segment': 'Guardrail enforcement',
  'use_when': 'Use after schema, drift, or DQ guardrail helpers to stop the notebook when '
  ...
  'side_effects': 'May terminate notebook execution through Fabric notebook utilities or raise an '
                  'exception.',
  'fabric_context': 'Use in 02_pipeline after validate_schema, monitor_data_changes, or '
  ...
  'preferred_example': 'schema_result = validate_schema(df, expected_schema)\n'
                       'stop_if_failed(schema_result)',
  'related_functions': ['validate_schema', 'monitor_data_changes', 'enforce_dq_rules']},
 {'kind': 'function',
 ...
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Read many pipeline source definitions into DataFrames.',
  'symbol_name': 'read_pipeline_sources',
  'template_notebook': '02_pipeline',
  'template_segment': 'Source reads',
  'use_when': 'Use in 02_pipeline after defining SOURCE_DEFINITIONS for one or more lakehouse, warehouse, or file sources.',
  'parameters': 'source_definitions plus config, env, and optional spark_session.',
  'returns': 'Dictionary of DataFrames keyed by source alias.',
  'side_effects': 'Reads source data through the configured Fabric routes.',
  'related_functions': ['profile_pipeline_datasets', 'read_lakehouse_table']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Profile many source or target datasets from definitions.',
  'symbol_name': 'profile_pipeline_datasets',
  'template_notebook': '02_pipeline',
  'template_segment': 'Profiling',
  'use_when': 'Use after reading sources or defining targets so profiles can feed drift and catalogue evidence.',
  'parameters': 'datasets, dataset_definitions, and include_distributions.',
  'returns': 'Dictionary of profile DataFrames keyed by dataset alias.',
  'related_functions': ['profile_dataframe', 'write_catalogue_evidence']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Run schema guardrails for many datasets using per-dataset presets.',
  'symbol_name': 'run_schema_guardrails',
  'template_notebook': '02_pipeline',
  'template_segment': 'Schema guardrails',
  'use_when': 'Use for source and target schema checks where each definition supplies expected_schema and schema_preset.',
  'parameters': 'datasets and dataset_definitions.',
  'returns': 'Dictionary of schema guardrail results keyed by dataset alias.',
  'related_functions': ['validate_schema', 'stop_if_failed']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Run data drift guardrails for many datasets using per-dataset presets.',
  'symbol_name': 'run_data_drift_guardrails',
  'template_notebook': '02_pipeline',
  'template_segment': 'Data drift guardrails',
  'use_when': 'Use for source and target drift checks where each definition supplies stage and drift_preset.',
  'parameters': 'datasets, dataset_definitions, spark, config, env, metadata_table, and optional run_id.',
  'returns': 'Dictionary of drift guardrail results keyed by dataset alias.',
  'related_functions': ['monitor_data_changes', 'write_catalogue_evidence']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Run approved DQ guardrails for many datasets using per-dataset presets.',
  'symbol_name': 'run_dq_guardrails',
  'template_notebook': '02_pipeline',
  'template_segment': 'DQ guardrails',
  'use_when': 'Use for source and target DQ checks where each definition supplies dq_preset.',
  'parameters': 'datasets, dataset_definitions, config, env, and optional spark_session.',
  'returns': 'Dictionary of DQ guardrail results keyed by dataset alias.',
  'related_functions': ['enforce_dq_rules', 'stop_if_failed']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Enrich profile evidence with guardrail context and write catalogue rows.',
  'symbol_name': 'write_catalogue_evidence',
  'template_notebook': '02_pipeline',
  'template_segment': 'Catalogue evidence',
  'use_when': 'Use after profiling and guardrails to persist source or target catalogue evidence through the metadata route.',
  'parameters': 'profiles, dataset_definitions, config, env, run_id, agreement context, and guardrail results.',
  ...
  'related_functions': ['profile_pipeline_datasets', 'write_lakehouse_table']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Add standard FabricOps runtime audit columns to target DataFrames.',
  'symbol_name': 'add_runtime_audit_columns',
  'template_notebook': '02_pipeline',
  'template_segment': 'Target definitions',
  'use_when': 'Use after defining target DataFrames and before target schema, drift, DQ, and write steps.',
  'parameters': 'datasets, run_id, pipeline_name, and optional created_at.',
  'returns': 'Dictionary of audited DataFrames keyed by target alias.',
  'related_functions': ['write_pipeline_targets']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Write many target DataFrames using target definitions.',
  'symbol_name': 'write_pipeline_targets',
  'template_notebook': '02_pipeline',
  'template_segment': 'Target writes',
  'use_when': 'Use only after target schema, drift, and DQ guardrails have passed or warned according to policy.',
  'parameters': 'targets, target_definitions, config, env, and default_mode.',
  'returns': 'Dictionary of target write statuses keyed by target alias.',
  'side_effects': 'Writes lakehouse or warehouse target tables through configured routes.',
  'related_functions': ['add_runtime_audit_columns', 'write_pipeline_run_summary']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  ...
  'related_functions': ['build_lineage_records', 'write_pipeline_run_summary']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  ...
  'related_functions': ['write_pipeline_lineage', 'write_catalogue_evidence']},
