...
PUBLIC_SYMBOL_DOCS = [
 ...
 {'kind': 'function',
  ...
  'function_type': 'callable',
  'summary_override': 'Review catalogue schemas, record exact contract schema versions, and maintain separate business-description enrichment.',
  'symbol_name': 'widget_register_data_contract',
  'template_notebook': '01_agreement',
  'template_segment': 'Agreement intake',
  'use_when': 'Use in 01_agreement to compare contracted and current catalogue schemas, edit business descriptions, and save contract membership and enrichment separately.',
  ...
  'returns': 'Mutable state with audit activity details, structured schema and enrichment editor rows, separate unsaved indicators and save results, and defensive-copy getters.',
  'raises': 'Raises when an agreement ID cannot be resolved or configured metadata cannot be read or safely written.',
  'related_functions': ['widget_render_data_agreement', 'widget_view_agreement_catalogue'],
  'expanded_purpose': 'An immutable inventory of logical datasets linked to a Data Agreement. Each logical catalogue dataset appears once, and selecting it previews the exact latest active-environment schema and fingerprint before addition. The widget resolves historical catalogue rows behind the contracted fingerprint, compares them with the current schema, and optionally shows removed contracted columns as read-only history using catalogue observation dates. The schema fingerprint is stored in METADATA_DATA_CONTRACT; table and current-column business descriptions are independently appended to METADATA_ENRICHMENT against stable catalogue identities.',
  'when_to_use': 'Use to select a logical dataset, review the schema that will be recorded, edit table and current-column descriptions, optionally inspect removed contracted columns, and invoke separate explicit contract and description saves.',
  'do_not_use_when': 'Do not use for guardrail execution, pipeline blocking, approval workflows, PII or sensitivity editing, or non-structural contract enforcement.',
  'glossary_terms': ['metadata catalogue', 'metadata lakehouse', 'data contract'],
  'return_interpretation': 'dataset_reviews exposes complete schemas and stable-key differences; enrichment editor state exposes current and removed description rows without HTML parsing. Contract saves record fingerprints while description saves append separate enrichment rows.',
