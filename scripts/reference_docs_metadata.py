Warning: truncated output (original token count: 48893)
Total output lines: 2465
...
METADATA_REFERENCE_OVERVIEW_INTRO = (
    ...
    "The ten implemented tables form the shared handoff layer across governance, data engineering, and AI and BI "
    "analytics work. These pages are generated from the implemented metadata setup schema registry used by "
    "`00_env_config`."
)
...
METADATA_REFERENCE_MODEL_DIAGRAM_CAPTION = (
    ...
    "profiling, catalogue, lineage, enrichment, access, guardrail intent, and guardrail result evidence."
)
...
PUBLIC_SYMBOL_DOCS = [
 ...
 {'kind': 'function',
  ...
  'fabric_context': 'Requires the FrameworkConfig or compatible CONFIG from 00_env_config plus the '
                  …18893 tokens truncated…mon_failure_causes": ["Zero or negative repartition counts, unsupported repartition_by types, empty lists/tuples, non-string column values after any leading partition count, or missing repartition columns.", "Invalid partition_by columns, schema mismatch, append-versus-overwrite conflicts, or unintended destructive overwrite.", "Insufficient write permissions, concurrent writes to the same target table, partial or failed Delta commits, empty DataFrame handling, small-file risk, or Spark shuffle failure."],
