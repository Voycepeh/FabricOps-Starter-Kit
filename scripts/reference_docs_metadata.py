...
PUBLIC_SYMBOL_DOCS = [
 {'kind': 'function',
  ...
  'summary_override': 'Detect source changes and plan incremental inspection ranges.',
  'purpose': 'Compare current and previously observed Spark DataFrames without choosing a target write policy.',
  'symbol_name': 'detect_source_changes',
  'template_notebook': '02_pipeline',
  'template_segment': 'Source change detection',
  'use_when': 'Use after an existing FabricOps reader obtains current and previous source states.',
  'do_not_use_when': 'Do not use to execute appends, merges, overwrites, SCD2, or stop/warn policy.',
  'parameters': 'current_df, previous_df, key_columns, incremental_column, refresh_days, source_pattern, version_columns, comparison_scope, and include_row_changes.',
  'returns': 'Structured source ranges, partition facts, row-change counts, flags, and optional row-level changes.',
  'raises': 'ValueError for invalid patterns, scopes, windows, columns, null keys, or ambiguous keys.',
  'side_effects': 'Runs Spark comparison actions; writes no data or metadata.',
  'fabric_context': 'Run on DataFrames returned by configured FabricOps readers for any supported source location.',
  'ai_verification': 'Confirm comparison scope is explicit and partial reads cannot be interpreted as deletions.',
  'preferred_example': 'changes = detect_source_changes(current_df, previous_df, key_columns=["order_id"], incremental_column="order_date", source_pattern="mutable_incremental")',
  'related_functions': ['profile_dataframe', 'profile_and_register_table'],
  'expanded_purpose': 'Establishes deterministic range, partition, and logical-row change facts for later orchestration.',
  'when_to_use': 'Use before a future target-specific write strategy decides how to respond to source changes.',
  'return_interpretation': 'Treat the result as observed facts, not as an instruction to write, warn, or stop.',
  'common_failure_causes': ['The comparison scope does not describe the supplied DataFrames.',
 ...
 {'kind': 'function',
  'module': 'widgets.widget_view_pipeline_catalogue',
  'function_type': 'callable',
  'summary_override': 'Select a Source or Target dataset linked to the current notebook through data lineage, then load its catalogue and profile DataFrames for native Fabric rendering.',
  'symbol_name': 'widget_view_pipeline_catalogue',
  'template_notebook': '02_pipeline',
  'template_segment': 'Pipeline catalogue',
  'use_when': 'Use in 02_pipeline to inspect Source or Target catalogue evidence from current-notebook lineage.',
  'related_functions': ['profile_and_register_table'],
  'return_interpretation': 'Call state["get_views"]() to receive exactly catalogue_df and profile_df for native Fabric display.'},
 {'kind': 'function',
  'module': 'widgets.widget_view_data_catalogue',
  'function_type': 'callable',
  'summary_override': 'Select any dataset recorded in the data catalogue, then load its catalogue and profile DataFrames for native Fabric rendering.',
  'symbol_name': 'widget_view_data_catalogue',
  'template_notebook': '99_explore',
  'template_segment': 'Data catalogue',
  'use_when': 'Use in 99_explore for read-only catalogue and profile inspection in the current environment.',
  'related_functions': ['profile_dataframe'],
  'return_interpretation': 'Call state["get_views"]() to receive exactly catalogue_df and profile_df for native Fabric display.'},
...
FOCUSED_FUNCTION_DOC_UPDATES = {
    ...
    "profile_and_register_table": {
        ...
        "returns": "Spark DataFrame containing one compact profiling summary row for each eligible column appended to METADATA_DATA_PROFILED, including stable identities, complete-DataFrame statistics, schema fingerprint, profiling timestamp, and runtime audit fields.",
        "return_interpretation": "The returned rows are the compact parent summaries. Flattened frequency rows are written separately to METADATA_DATA_PROFILED_FREQUENCY and join to the returned rows through metadata_column_key; frequency, catalogue, and lineage rows are side effects and are not returned.",
