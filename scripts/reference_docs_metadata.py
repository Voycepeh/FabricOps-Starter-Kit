...
PUBLIC_SYMBOL_DOCS = [
 ...
 {'kind': 'function',
  ...
  'function_type': 'callable',
  'summary_override': 'Review catalogue schemas and accept immutable agreement inventory baselines.',
  ...
  'expanded_purpose': 'An immutable inventory of logical datasets linked to a Data Agreement. Each logical catalogue dataset appears once, and selecting it previews the exact current schema and fingerprint before addition. The widget resolves historical catalogue rows behind the contracted fingerprint, compares them with the current active-environment schema, and displays additive or breaking structural differences. Each explicit save accepts one fingerprint per agreement_id + metadata_table_key under one shared _activity_id while _committed_at orders the append-only snapshots.',
  'when_to_use': 'Use to review actual contracted and current schemas, add or remove logical datasets, and explicitly accept the complete resulting inventory as one snapshot.',
  ...
  'return_interpretation': 'dataset_reviews exposes complete schemas and stable-key differences for the editable latest inventory; each save appends one complete non-empty baseline without changing history.',
