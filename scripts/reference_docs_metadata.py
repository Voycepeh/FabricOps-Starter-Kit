...
METADATA_REFERENCE_OVERVIEW_INTRO = """FabricOps uses one Metadata Lakehouse with two physical schemas, while keeping its own metadata separate from the physical data that a project produces or chooses to retain. The canonical `table_id` identifies the governed data asset without making every related record a direct child of that field.
...
METADATA_REFERENCE_MODEL_DIAGRAM_CAPTION = (
    "The current-state physical diagram below reflects the implemented metadata relationships, including the "
    "temporary direct `table_id` fields on Enrichment and Guardrails. The intended contract-owned relationship "
    "described above will replace those links in the separate schema migration."
)
...
METADATA_COLUMN_OWNERS = {
    ...
    "METADATA_DATA_ACCESS": {
        "__default__": [
            "fabricops_kit.access.scan_warehouse_access.scan_warehouse_access",
            "fabricops_kit.access.scan_warehouse_access._access_rows",
        ],
        "__audit__": ["fabricops_kit.config.audit.build_runtime_audit_fields"],
        "access_id": ["fabricops_kit.access.scan_warehouse_access._access_rows"],
        "table_id": ["fabricops_kit.access.scan_warehouse_access._map_to_catalogue"],
        "access_snapshot_id": ["fabricops_kit.access.scan_warehouse_access.scan_warehouse_access"],
        "permission_source": ["fabricops_kit.access.scan_warehouse_access._scan_targets"],
        "database_name": ["fabricops_kit.access.scan_warehouse_access._scan_targets"],
        "schema_name": ["fabricops_kit.access.scan_warehouse_access._scan_targets"],
        "object_name": ["fabricops_kit.access.scan_warehouse_access._scan_targets"],
        "object_type": ["fabricops_kit.access.scan_warehouse_access._scan_targets"],
    },
    "METADATA_ENRICHMENT": {
        "__default__": [
...
PUBLIC_SYMBOL_DOCS = [
 {'kind': 'function',
  ...
  'summary_override': 'Scan configured Fabric Warehouses for observable SQL permissions and link them to governed tables.',
  ...
  'symbol_name': 'scan_warehouse_access',
  'template_notebook': '90_access_inventory (planned)',
  'template_segment': 'Access inventory',
  'use_when': 'Use when you need a snapshot of observable Warehouse SQL permissions for tables already registered in METADATA_DATA_CATALOGUE.',
  ...
  'parameters': 'catalogue_df, one or more configured Warehouse target keys, optional environment_name, optional access_snapshot_id, optional spark_session, and optional FabricOps context.',
  ...
  'raises': 'Raises ValueError when no valid Warehouse target is supplied and propagates configured Warehouse read or Spark errors.',
  'side_effects': 'Reads SQL permission catalogue views from configured Warehouse targets; it does not persist access rows or change permissions.',
  'fabric_context': 'Requires configured Warehouse targets and a Catalogue DataFrame for the same FabricOps environment.',
  'ai_verification': 'Confirm targets match the Catalogue layer values, inspect unmatched observations, and persist only the returned access DataFrame when appropriate.',
  'preferred_example': 'result = scan_warehouse_access(catalogue_df, targets="warehouse", spark_session=spark)',
  'related_functions': ['read_warehouse_query'],
  'expanded_purpose': 'Uses the existing read-only Warehouse query helper, preserves direct versus role-based permission evidence, expands schema/database scope across registered tables, and keeps unresolved observations visible.',
  ...
  'common_failure_causes': ['A Warehouse target is not configured or accessible.', 'Catalogue layer/schema/table identity does not match the observed SQL object.', 'The scanning identity cannot read the SQL permission catalogue views.'],
