...
TEMPLATE_FLOW_DOCS = [{'notebook_key': '00_env_config',
 ...
 {'notebook_key': '02_pipeline',
  ...
  'segments': [{'symbols': ['widget_select_data_contract'],
               ...
               {'symbols': ['read_pipeline_prep', 'check_schema', 'check_freshness', 'check_source_stability', 'check_dq'],
...
PUBLIC_SYMBOL_DOCS = [
 ...
 {'kind': 'function',
  ...
  'use_when': 'Use after write_pipeline_prep resolves target identity and before the physical governed write.',
  ...
  'related_functions': ['check_dq', 'write_pipeline_prep', 'write_pii_token_map', 'stop_if_failed']},
 {'kind': 'function',
  'module': 'pipeline.read_pipeline_prep',
  'function_type': 'callable',
  'summary_override': 'Resolve governed source identity and register source Lineage before reading business data.',
  'symbol_name': 'read_pipeline_prep',
  'template_notebook': '02_pipeline',
  'template_segment': 'Source preparation',
  'use_when': 'Use before the visible source reader to resolve one registered source and register source Lineage.',
  'do_not_use_when': 'Do not use as a replacement for the physical Lakehouse or SQL endpoint reader.',
  'parameters': 'Canonical source table_id or configured physical source target, schema, and table.',
  'returns': 'Canonical source table_id and resolved physical source identity.',
  'side_effects': 'Persists source Lineage; it does not read business rows or write the target.',
  'preferred_example': 'read_prep = read_pipeline_prep(source_table_id=source["table_id"])',
  'related_functions': ['write_pipeline_prep', 'read_lakehouse_table', 'read_warehouse_table']},
 {'kind': 'function',
  ...
  'related_functions': ['write_pipeline_prep', 'observe_table', 'check_schema']},
 {'kind': 'function',
  ...
  'use_when': 'Use after engineering transformation to resolve target identity and validate proposed processing before target Guardrails and the visible writer.',
  'do_not_use_when': 'Do not use as a replacement for write_lakehouse_table or write_warehouse_table.',
  'parameters': 'Business DataFrame, target identity or physical coordinates, engineering-authored load strategy and parameters, and source preparation results.',
  'returns': 'Audited DataFrame, target identity, authored and governed processing context, resolved mode, writer settings, write scope, and post-write success context.',
 ...
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Profile a Spark DataFrame, save a profiling snapshot, and update catalogue records.',
  ...
  'side_effects': 'Saves a new profiling snapshot to METADATA_DATA_PROFILED and updates or adds table and column records in METADATA_DATA_CATALOGUE.',
  ...
  'expanded_purpose': 'Orchestrates DataFrame profiling, threshold-guarded default frequency evidence, canonical profiled schema mapping, catalogue identity derivation, deterministic key creation, and append-only profiled evidence registration and catalogue identity upsert while accepting profile_role as execution participation context. The role is validated but not stored in METADATA_DATA_PROFILED or METADATA_DATA_CATALOGUE; Lineage is recorded by governed pipeline preparation and successful publication.',
  'when_to_use': 'Use in 02_pipeline once for each DataFrame that should produce profiled evidence and catalogue identity. Pass profile_role as execution participation context; Lineage is recorded by governed pipeline preparation and successful publication.',
  'glossary_terms': ['evidence', 'source data', 'target table'],
 ...
 {'kind': 'function',
  ...
  'related_functions': ['profile_and_register_table'],
 ...
 {'kind': 'function',
  ...
  'related_functions': ['widget_activate_data_contract', 'read_pipeline_prep', 'check_schema', 'check_freshness', 'check_source_stability', 'check_dq'],
...
FOCUSED_FUNCTION_DOC_UPDATES = {
    ...
    "profile_and_register_table": {
        "expanded_purpose": "Profiles the supplied Spark DataFrame exactly as provided; saves compact summary rows to METADATA_DATA_PROFILED; reuses profile_frequency_distribution to replace normalized rows in METADATA_DATA_PROFILED_FREQUENCY; and updates catalogue identities through the configured metadata lakehouse.",
