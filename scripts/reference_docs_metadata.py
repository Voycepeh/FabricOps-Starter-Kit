...
TEMPLATE_FLOW_DOCS: list[TemplateFlowDocMetadata] = [{'notebook_key': '00_env_config',
 ...
 {'notebook_key': '02_pipeline',
  ...
  'segment_intro': 'Thin production orchestration that keeps beginner-editable configs, transform logic, lineage relationships, and pipeline naming visible while package helpers handle reusable setup, guardrails, writes, and evidence plumbing.',
...
PUBLIC_SYMBOL_DOCS: list[PublicSymbolDocMetadata] = [{'kind': 'function',
 ...
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Enrich source table configs and load source DataFrames for 02_pipeline.',
  'symbol_name': 'prepare_source_table_configs',
  'template_notebook': '02_pipeline',
  'template_segment': 'Source preparation',
  'use_when': 'Use after SOURCE_TABLES and DEFAULT_SOURCE_GUARDRAILS are defined to derive source defaults, load DataFrames, and build SOURCE_CONFIG_BY_KEY.',
  'do_not_use_when': 'Do not use for one-off interactive reads where no FabricOps guardrail configuration is needed.',
  'parameters': 'source_table_configs, default_source_guardrails, config, env, and spark_session.',
  'returns': 'Enriched source configs and a dictionary keyed by source key.',
  ...
  'fabric_context': 'Uses CONFIG and env from 00_env_config so source reads follow configured Fabric targets.',
  'ai_verification': 'Verify every source has a unique key and the derived configs are passed to run_table_guardrails before transformation.',
  'preferred_example': 'SOURCE_TABLES, SOURCE_CONFIG_BY_KEY = prepare_source_table_configs(SOURCE_TABLES, DEFAULT_SOURCE_GUARDRAILS, CONFIG, ENV_NAME, spark)',
  'related_functions': ['read_lakehouse_table', 'run_table_guardrails']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Enrich target table configs and add FabricOps audit columns for 02_pipeline.',
  'symbol_name': 'prepare_target_table_configs',
  'template_notebook': '02_pipeline',
  'template_segment': 'Target preparation',
  'use_when': 'Use after TARGET_TABLES and DEFAULT_TARGET_GUARDRAILS_AND_WRITE_OPTIONS are defined to add audit columns, derive target defaults, and build TARGET_CONFIG_BY_KEY.',
  'do_not_use_when': 'Do not use before business target DataFrames have been created in the transform section.',
  'parameters': 'target_table_configs, default_target_guardrails_and_write_options, run_id, and pipeline_name.',
  'returns': 'Enriched target configs and a dictionary keyed by target key.',
  'side_effects': 'Adds FabricOps audit columns to target DataFrames; it does not write targets.',
  'fabric_context': 'Run inside Fabric/Spark after target DataFrames exist and before target guardrails run.',
  'ai_verification': 'Verify run_table_guardrails receives the enriched TARGET_TABLES and write_target_tables runs only after stop_if_any_guardrail_failed passes.',
  'preferred_example': 'TARGET_TABLES, TARGET_CONFIG_BY_KEY = prepare_target_table_configs(TARGET_TABLES, DEFAULT_TARGET_GUARDRAILS_AND_WRITE_OPTIONS, RUN_ID, PIPELINE_NAME)',
  'related_functions': ['run_table_guardrails', 'write_target_tables']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Write checked target DataFrames from TARGET_TABLES to Fabric targets.',
  'symbol_name': 'write_target_tables',
  'template_notebook': '02_pipeline',
  'template_segment': 'Target writes',
  'use_when': 'Use after target guardrails pass to write the checked or DQ-annotated DataFrame held in each target config.',
  'do_not_use_when': 'Do not call before stop_if_any_guardrail_failed has validated target guardrail results.',
  'parameters': 'target_table_configs, config, and env.',
  'returns': 'Write status dictionary keyed by target key.',
  'side_effects': 'Writes Lakehouse or Warehouse target tables through configured Fabric targets.',
  'fabric_context': 'Uses CONFIG and env from 00_env_config and supports lakehouse and warehouse target kinds.',
  'ai_verification': 'Verify target_kind, target_layer, target_name, write_mode, and optional partition/write settings match the intended Fabric target.',
  'preferred_example': 'target_write_status = write_target_tables(TARGET_TABLES, CONFIG, ENV_NAME)',
  'related_functions': ['prepare_target_table_configs', 'write_lakehouse_table', 'write_warehouse_table']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Build catalogue evidence definitions for pipeline table guardrails.',
  'symbol_name': 'build_guardrail_evidence_definitions',
  'template_notebook': '02_pipeline',
  'template_segment': 'Guardrail orchestration',
  'use_when': 'Use inside pipeline guardrail orchestration to convert source or target table configs into catalogue evidence definitions.',
  'do_not_use_when': 'Most notebook authors should not call this directly; use run_table_guardrails for normal 02_pipeline checks.',
  'parameters': 'table_configs containing source or target table definitions and DataFrames.',
  'returns': 'Dictionary of catalogue evidence definitions keyed by table key with DataFrame values removed.',
  'side_effects': 'Pure helper; it does not read or write metadata.',
  'related_functions': ['run_table_guardrails', 'write_catalogue_evidence']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  ...
  'returns': 'Guardrail result bundle with profiles, schema results, stability results, DQ results, catalogue status, evidence definitions, can_continue, and failed_tables.',
  ...
  'ai_verification': 'Verify stop_if_any_guardrail_failed is called after displaying or inspecting results and before transformation or writes continue.',
  'preferred_example': 'source_guardrail_results = run_table_guardrails(SOURCE_TABLES, config=CONFIG, env=ENV_NAME, run_id=RUN_ID, spark_session=spark)',
  'related_functions': ['guardrail_summary', 'stop_if_any_guardrail_failed', 'write_catalogue_evidence']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Return a concise display summary for table guardrail results.',
  'symbol_name': 'guardrail_summary',
  'template_notebook': '02_pipeline',
  'template_segment': 'Guardrail summary',
  'use_when': 'Use after run_table_guardrails to display schema, stability, DQ, catalogue, and failed-table results without exposing internal profile objects.',
  'parameters': 'guardrail_results returned by run_table_guardrails.',
  'returns': 'Dictionary containing schema_results, stability_results, dq_results, catalogue_status, and failed_tables.',
  'side_effects': 'Pure formatting helper; it does not display by itself or write metadata.',
  'related_functions': ['run_table_guardrails']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
  'summary_override': 'Stop notebook execution when any table guardrail is blocking.',
  'symbol_name': 'stop_if_any_guardrail_failed',
  'template_notebook': '02_pipeline',
  'template_segment': 'Guardrail stopping',
  'use_when': 'Use immediately after displaying source or target guardrail results to block transformation or writes when any table cannot continue.',
  'parameters': 'guardrail_results returned by run_table_guardrails.',
  'returns': 'None when all guardrails can continue; raises through stop_if_failed for blocking failures.',
  'side_effects': 'May stop notebook execution by delegating to stop_if_failed.',
  'related_functions': ['run_table_guardrails', 'guardrail_summary', 'stop_if_failed']},
 {'kind': 'function',
  'module': 'pipeline',
  'function_type': 'callable',
