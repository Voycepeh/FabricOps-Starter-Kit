...
FOCUSED_FUNCTION_DOC_UPDATES = {
    ...
    "profile_and_register_dataframe": {
        ...
        "return_interpretation": "The returned rows are the detailed profile results for eligible columns. Statistical metrics always describe the complete supplied DataFrame. When frequency_sample_rows is supplied, frequency counts and percentages describe the sampled frequency input only if sampling actually reduces the row population, and frequency_json discloses the full source row count, profiled row count, whether sampling was requested, whether it was applied, requested sample size, method, and seed. Catalogue rows and source or target activity records are saved as side effects and are not returned.",
        "common_failure_causes": ["profile_role must be source or target and store_type must be lakehouse or warehouse.", "environment_name, layer, or table_name is blank, or schema_name is blank when supplied.", "frequency_sample_rows is not a positive integer when supplied or frequency_sample_seed is not an integer.", "The configured metadata target cannot be resolved or written.", "Requested frequency columns are missing or expensive to group; frequency_top_n limits returned values only and does not reduce grouping cost.", "If lineage registration fails after profile and catalogue writes succeed, RuntimeError is raised and earlier writes remain completed."],
        "preferred_example": 'profiled_df = profile_and_register_dataframe(source_df, profile_role="source", environment_name=ENVIRONMENT_NAME, store_type="lakehouse", layer="raw", table_name="student_enrolment", frequency_sample_rows=100_000, frequency_sample_seed=42)',
    },
}
