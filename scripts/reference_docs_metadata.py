...
FOCUSED_FUNCTION_DOC_UPDATES = {
    ...
    "write_lakehouse_table": {
        ...
        "parameters": "df is the Spark DataFrame to write and is not mutated; table_name is the unqualified Lakehouse table; target resolves the lakehouse; schema optionally qualifies schema-enabled lakehouses; mode controls append/overwrite/errorifexists/ignore behavior; partition_by physically partitions Delta files and folders; repartition_by accepts a positive integer, one column name, or a non-empty list/tuple of column names and calls Spark repartitioning before writing; options are forwarded to the Delta writer; verbose prints the resolved path; context supplies FabricOps config/env.",
    ...
    "write_warehouse_table": {
        ...
        "parameters": "df is the Spark DataFrame to transfer and is not mutated; schema is the Warehouse schema such as dbo; table_name is the Warehouse table; target resolves the Warehouse connection; mode is passed to the connector; repartition_by accepts a positive integer, one column name, or a non-empty list/tuple of column names and calls Spark repartitioning before connector transfer; options are forwarded to the connector after required identity options; context supplies FabricOps config/env. No partition_by parameter exists for Warehouse writes.",
        ...
        "common_failure_causes": ["Zero or negative repartition counts, missing repartition columns, unsupported repartition_by value types, or empty lists/tuples.", "Schema or table not found, unsupported write mode, authentication or connector failure, Warehouse permission failure, or unsupported Spark-to-Warehouse type conversion.", "Connector staging or cleanup failure, transaction or lock conflict, empty DataFrame behaviour, large transfer timeout/resource exhaustion, or accidentally writing the original DataFrame instead of the repartitioned one."],
    ...
    "profile_dataframe": {
        ...
        "when_to_use": "Use for exploratory or governed profiling when you need a reusable profile DataFrame but do not want to write metadata. Use profile_and_register_dataframe when the profile must be persisted as FabricOps evidence.",
    ...
    "profile_frequency_distribution": {
        "expanded_purpose": "Calculates exact top-N value frequency distributions for selected Spark DataFrame columns, returning counts, percentages, ranks, source data type, total row count, and non-null count. It does not sample or write metadata.",
        ...
        "returns": "Spark DataFrame containing up to top_n ranked frequency rows per profiled column. Null is included as a value and non-null counts are reported separately.",
        "common_failure_causes": ["top_n is not greater than zero.", "Requested columns are missing.", "No eligible scalar columns are available when columns is omitted.", "High-cardinality columns require expensive grouped counts before the top-N limit is applied."],
        "preferred_example": 'frequency_df = profile_frequency_distribution(source_df, columns=["enrolment_status", "programme_code"], top_n=10)',
    },
    "profile_and_register_dataframe": {
        "expanded_purpose": "Profiles the supplied Spark DataFrame exactly as provided, appends detailed profile evidence to METADATA_DATA_PROFILED, upserts physical table and column identities into METADATA_DATA_CATALOGUE, and writes one table-level lineage participation event to METADATA_DATA_LINEAGE through the configured metadata lakehouse.",
        "when_to_use": "Use once for each source or target DataFrame that should produce persisted profiling evidence and catalogue identity in a pipeline run. Use profile_dataframe alone when you only need an in-memory profile.",
        "returns": "Spark DataFrame containing the canonical detailed profiling rows appended to METADATA_DATA_PROFILED, including physical asset identity, statistical metrics, optional frequency_json, schema fingerprint, sampling provenance, and runtime audit fields.",
        ...
        "preferred_example": 'profiled_df = profile_and_register_dataframe(source_df, profile_role="source", environment_name=ENVIRONMENT_NAME, store_type="lakehouse", layer="raw", table_name="student_enrolment", frequency_columns=["enrolment_status"], frequency_top_n=10)',
    },
}
