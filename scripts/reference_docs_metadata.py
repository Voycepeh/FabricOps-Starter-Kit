...
FOCUSED_FUNCTION_DOC_UPDATES = {
    ...
    "profile_and_register_table": {
        "expanded_purpose": "Profiles the supplied Spark DataFrame and uses its governed processing scope to keep incremental subsets diagnostic while registering complete physical-table profiles in METADATA_DATA_PROFILED, METADATA_DATA_PROFILED_FREQUENCY, and METADATA_DATA_CATALOGUE.",
        "when_to_use": "Use after an explicit physical read and explicit Guardrail calls. Pass read_pipeline_prep()[\"scope\"] for a source so incremental subsets remain diagnostic, and set register_profile=False when a full query result does not represent the complete registered physical table.",
