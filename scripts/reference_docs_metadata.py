...
PUBLIC_SYMBOL_DOCS = [
 ...
 {'kind': 'function',
  ...
  'parameters': 'df, environment_name, store_type, layer, table_name, optional schema_name, optional frequency_columns, frequency_top_n, and is_sampled.',
  'returns': 'Final Spark DataFrame appended to METADATA_DATA_CATALOGUE.',
  'raises': 'Raises ValueError for unsupported store_type or empty required identity fields; lower-level profiling validation is preserved.',
  ...
  'ai_verification': 'Verify the physical asset identity, role, optional frequency columns, and append-only metadata write before running.',
  'preferred_example': 'catalogue_profile_df = profile_and_register_dataframe(df_customer, environment_name=ENVIRONMENT_NAME, store_type="lakehouse", layer="raw", table_name="customer")',
  'related_functions': ['profile_dataframe', 'profile_frequency_distribution', 'run_table_guardrails'],
  'expanded_purpose': 'Orchestrates DataFrame profiling, optional top-N frequency evidence, canonical catalogue schema mapping, deterministic key creation, and append-only metadata registration while keeping the lower-level profiling helpers independently usable.',
  'when_to_use': 'Use in 02_pipeline once for each DataFrame that should produce catalogue profile evidence; register exact source-to-target relationships separately through lineage.',
  'glossary_terms': ['evidence', 'source data', 'target table'],
