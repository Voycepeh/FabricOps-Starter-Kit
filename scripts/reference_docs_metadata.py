...
PUBLIC_SYMBOL_DOCS = [
 ...
 {'kind': 'function',
  ...
  'function_type': 'callable',
  'summary_override': 'Review catalogue schemas and record exact schema versions in immutable agreement inventories.',
  'symbol_name': 'widget_register_data_contract',
  'template_notebook': '01_agreement',
  'template_segment': 'Agreement intake',
  'use_when': 'Use in 01_agreement to compare contracted and current catalogue schemas before saving a complete new immutable inventory snapshot.',
  ...
  'expanded_purpose': 'An immutable inventory of logical datasets linked to a Data Agreement. Each logical catalogue dataset appears once, and selecting it previews the exact current schema and fingerprint before addition. The widget resolves historical catalogue rows behind the contracted fingerprint, compares them with the current active-environment schema, and displays additive or breaking structural differences. Each explicit save records one fingerprint per agreement_id + metadata_table_key under one shared _activity_id while _committed_at orders the append-only snapshots.',
  'when_to_use': 'Use to review actual contracted and current schemas, add or remove logical datasets, and explicitly record the complete resulting inventory as one snapshot.',
  'do_not_use_when': 'Do not use for enrichment, guardrail execution, pipeline blocking, cross-environment authoring, or non-structural contract enforcement.',
  'glossary_terms': ['metadata catalogue', 'metadata lakehouse', 'data contract'],
  'return_interpretation': 'dataset_reviews exposes complete schemas and stable-key differences for the editable latest inventory; each save records one exact schema version per logical dataset without changing history.',
