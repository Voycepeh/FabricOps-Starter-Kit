...
TEMPLATE_FLOW_DOCS: list[TemplateFlowDocMetadata] = [{'notebook_key': '00_env_config',
 ...
 {'notebook_key': '02_pipeline',
  ...
  'segment_intro': 'Thin production orchestration that keeps normal reads, profiling, guardrails, DQ checks, and writes visible while hiding metadata evidence plumbing.',
...
PUBLIC_SYMBOL_DOCS: list[PublicSymbolDocMetadata] = [{'kind': 'function',
 ...
 {'kind': 'function',
  ...
  'related_functions': ['generate_schema_guardrail_config', 'print_schema_guardrail_config', 'display_schema_profile', 'enforce_catalogue_stability', 'stop_if_failed']},
 {'kind': 'function',
  'module': 'drift',
  'function_type': 'callable',
  'summary_override': 'Generate a reviewed starter expected_schema dictionary from a DataFrame schema.',
  'symbol_name': 'generate_schema_guardrail_config',
  'template_notebook': '02_pipeline',
  'template_segment': 'Schema validation',
  'use_when': 'Use before authoring expected_schema to draft a starter schema guardrail from the current DataFrame schema.',
  'do_not_use_when': 'Do not treat the generated dictionary as approved without human review.',
  'parameters': 'dataframe plus optional exclude_columns and sort_columns.',
  'returns': 'Python dictionary mapping column names to normalized guardrail datatypes.',
  'raises': 'Raises DataFrame/schema inspection errors when schema metadata is unavailable.',
  'side_effects': 'Inspects schema metadata only; it does not write metadata, tables, or files.',
  'fabric_context': 'Use in 02_pipeline helper cells to bootstrap expected_schema before validate_schema.',
  'ai_verification': 'Verify excluded columns, nullable business expectations, and normalized types before copying into pipeline guardrails.',
  'preferred_example': 'expected_schema = generate_schema_guardrail_config(df, exclude_columns=["_fabricops_run_id"])',
  'related_functions': ['display_schema_profile', 'print_schema_guardrail_config', 'validate_schema']},
 {'kind': 'function',
  'module': 'drift',
  'function_type': 'callable',
  'summary_override': 'Print copy-paste-ready starter expected_schema code from a DataFrame schema.',
  'symbol_name': 'print_schema_guardrail_config',
  'template_notebook': '02_pipeline',
  'template_segment': 'Schema validation',
  'use_when': 'Use in notebooks when a user wants copy-paste-ready expected_schema starter code.',
  'do_not_use_when': 'Do not use as an approval step; users still need to review the generated schema.',
  'parameters': 'dataframe plus optional exclude_columns, sort_columns, and variable_name.',
  'returns': 'The generated schema guardrail dictionary after printing Python code.',
  'raises': 'Raises DataFrame/schema inspection errors when schema metadata is unavailable.',
  'side_effects': 'Prints a Python dictionary snippet; it does not write metadata, tables, or files.',
  'fabric_context': 'Use in 02_pipeline helper cells before filling SOURCE_DATASETS or TARGET_DATASETS expected_schema.',
  'ai_verification': 'Confirm the printed dictionary was reviewed before pasting into approved guardrails.',
  'preferred_example': 'print_schema_guardrail_config(df, sort_columns=True)',
  'related_functions': ['generate_schema_guardrail_config', 'display_schema_profile', 'validate_schema']},
 {'kind': 'function',
  'module': 'drift',
  'function_type': 'callable',
  'summary_override': 'Display current schema columns, Spark datatypes, nullable flags, and proposed guardrail datatypes.',
  'symbol_name': 'display_schema_profile',
  'template_notebook': '02_pipeline',
  'template_segment': 'Schema validation',
  'use_when': 'Use in notebooks to review current schema details before accepting a starter expected_schema.',
  'do_not_use_when': 'Do not use as a replacement for validate_schema; it is a review aid only.',
  'parameters': 'dataframe plus optional exclude_columns and sort_columns.',
  'returns': 'List of row dictionaries containing column_name, spark_data_type, nullable, and guardrail_data_type.',
  'raises': 'Raises DataFrame/schema inspection errors when schema metadata is unavailable.',
  'side_effects': 'Prints a readable schema profile table; it does not write metadata, tables, or files.',
  'fabric_context': 'Use in 02_pipeline optional helper cells before manual schema guardrail configuration.',
  'ai_verification': 'Check that proposed datatypes and excluded technical columns match the intended contract.',
  'preferred_example': 'display_schema_profile(df, exclude_columns=["_fabricops_run_id"])',
  'related_functions': ['generate_schema_guardrail_config', 'print_schema_guardrail_config', 'validate_schema']},
 {'kind': 'function',
  'module': 'drift',
  'function_type': 'callable',
