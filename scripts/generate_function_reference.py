...
def _function_link(symbol: str, relative_prefix: str = "../") -> str:
    """Return a markdown link for public function pages, or code for support functions."""
    if symbol not in V1_CALLABLES:
        return f"`{symbol}`"
...
def main() -> None:
    ...
    for row in module_sidebar_rows:
        ...
        if module_name not in discovered_set:
            raise RuntimeError(f"Notebook sidebar module is missing in src/fabricops_kit: {module_name}")
    ...
    ref = [
        ...
        "",
        f"FabricOps exposes {len(function_symbol_map)} public Starter Kit functions backed by {sum(1 for n in node_by_qn.values() if not n['exported'])} supporting internal functions.",
        "",
    ...
    all_items: list[str] = []
    function_category_by_name = {name: "starter-kit-public" for name in function_symbol_map}
    ...
    for qn, node in sorted(node_by_qn.items()):
        ...
        manifest_category = function_category_by_name.get(short_name, "supporting-internal")
