...
function refactorPacket() { const callables=selectedItems().map(exportItem); return {export_type:'fabricops_callable_refactor_packet', generated_from:'Callable Inventory and Refactor Signals', selected_count:callables.length, filters:exportFilters(), refactor_context:refactorContext(callables), ai_prompt:AI_PROMPT, callables, functions:callables}; }
