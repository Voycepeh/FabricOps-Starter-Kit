...
FUNCTION_CALL_GRAPH_DASHBOARD_META_PATH = REFERENCE_DATA_DIR / "function-call-graph-dashboard-meta.json"
...
def _callable_flow_dashboard_metadata(flow_data: dict[str, Any]) -> dict[str, Any]:
    """Return lightweight dashboard metadata for the standalone HTML shell."""
    metadata = dict(flow_data.get("metadata", {}))
    return {
        "data_source": "function-call-graph.json",
        "source_generated_at_utc": metadata.get("generated_at_utc", ""),
        "dashboard_ui_generated_at_utc": metadata.get("dashboard_ui_generated_at_utc")
...
def _render_callable_generation_banner(flow_data: dict[str, Any] | None = None) -> str:
    """Render the dashboard metadata banner shell populated by JavaScript."""
    del flow_data
    return (
        '<p id="callableGenerationBanner" class="generation-banner">'
        '<strong>Data source generated at:</strong> <span data-dashboard-meta="source_generated_at_utc">Loading… SGT</span>'
        '<span><strong>Dashboard UI generated at:</strong> '
        '<span data-dashboard-meta="dashboard_ui_generated_at_utc">Loading… SGT</span></span>'
        '<span hidden><strong>Branch:</strong> <span data-dashboard-meta="branch"></span></span>'
        '<span hidden><strong>Commit:</strong> <span data-dashboard-meta="commit"></span></span>'
        '</p>'
    )
...
def _trim_callable_flow_dashboard_contract(
    ...
    for flow in callable_flow_data["public_entrypoint_flow"]:
        ...
        if node_by_qn is not None and module_data is not None and flow.get("qualified_name"):
            trimmed_flow["selected_flow_tree_html"] = "\n".join(
                _render_callable_architecture_flow_tree(flow, node_by_qn, module_data)
            )
...
def _render_dashboard_architecture_scope_rows(flow_data: dict[str, Any]) -> str:
    ...
    for index, flow in enumerate(public_flows):
        ...
        rows.append(
            f'<tr data-public-flow-row="{qn}" data-public-flow-summary="{summary}" data-public-flow-search="{html.escape(" ".join([str(flow.get("function_name") or ""), str(flow.get("qualified_name") or ""), " ".join(signals)]).lower(), quote=True)}" class="{active} severity-{severity}-row" tabindex="0" aria-selected="false">'
...
def main() -> None:
    ...
    FUNCTION_CALL_GRAPH_DASHBOARD_META_PATH.write_text(
        json.dumps(_callable_flow_dashboard_metadata(callable_flow_data), indent=2) + "\n",
        encoding="utf-8",
    )
