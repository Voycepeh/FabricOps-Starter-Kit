...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .filter-panel { display: grid; grid-template-columns: minmax(240px, 1.3fr) minmax(150px, .7fr) minmax(160px, .75fr) minmax(190px, .85fr) minmax(210px, 1fr) auto; gap: .55rem; align-items: end; margin: .75rem 0 1rem; padding: .75rem; border: 1px solid var(--border); border-radius: .9rem; background: #f1f5f9; }
...
<main>
  ...
  <section class="filter-panel" aria-label="Inventory filters">
    ...
    <label class="filter-field">Callable kind <select id="kindFilter"><option value="">All callable kinds</option></select></label>
  ...
  <p id="resultCount"></p>
  <section class="table-wrap" aria-label="Combined callable inventory"><table><thead><tr><th><input id="selectAllVisible" type="checkbox" aria-label="Select all visible rows"></th><th></th><th><button type="button" data-sort="function_name">Callable</button></th><th><button type="button" data-sort="callable_kind">Callable kind</button></th><th><button type="button" data-sort="function_type">Callable layer</button></th><th><button type="button" data-sort="review_status_label">Review status</button></th><th><button type="button" data-sort="module">Module</button></th><th class="num"><button type="button" data-sort="used_by_count">Used by</button></th><th class="num"><button type="button" data-sort="calls_count">Calls</button></th><th>Signal</th><th>Recommended action</th><th>Source</th></tr></thead><tbody id="inventoryBody"></tbody></table></section>
</main>
<script>
...
const priorityRank = {High:0, Medium:1, Low:2, Review:3, Protect:4}; const state = {search:'', type:'', kind:'', review_status:'', signal:'', consistency:'', compatibility_mode:'internal_cleanup', sortKey:'function_type', sortDir:1, expanded:new Set(), selected:new Set(), activeTree:''};
...
function exportFilters() { return {search_function_name:state.search, callable_kind:state.kind, callable_layer:state.type, review_status:state.review_status, recommended_action:state.signal}; }
...
function renderTable() { const rows=filteredRows().sort(compare); renderExportToolbar(rows); $('resultCount').textContent=`Showing ${rows.length} of ${inventory.length} discovered callables.`; $('inventoryBody').innerHTML=rows.map(i=>{ const id=i.qualified_name, open=state.expanded.has(id), t=text(i.function_type).toLowerCase(); return `<tr><td><input type="checkbox" data-select-row="${esc(id)}" aria-label="Select ${esc(i.function_name)}" ${state.selected.has(id)?'checked':''}></td><td><button type="button" class="action-cell" data-toggle="${esc(id)}" aria-expanded="${open}">${open?'Hide':'View'}</button></td><td class="function-name" title="${esc(i.qualified_name)}"><a href="${esc(i.source_url || '#')}"><code>${esc(i.function_name)}</code></a><br><small>${esc(i.qualified_name)}</small></td><td><span class="badge review">${esc(text(i.callable_kind).replace(/^./, c=>c.toUpperCase()))}</span></td><td><span class="badge ${t}">${esc(i.function_type)}</span></td><td><span class="badge review">${esc(i.review_status_label || i.review_status)}</span></td><td><code>${esc(i.module)}</code></td><td class="num">${esc(i.used_by_count ?? i.called_by_count)}</td><td class="num">${esc(i.calls_count)}</td><td>${chips(i)}</td><td><span class="chip-wrap"><span class="badge review" title="${esc(i.layer_consistency_label || '')}">${esc(i.recommended_action)}</span></span></td><td class="source-cell"><a class="source-link" href="${esc(i.source_url || '#')}">${esc(i.source_path || 'Source unavailable')}</a></td></tr>${open?`<tr class="details"><td colspan="12"><div class="details-panel"><p class="signal-reason"><strong>Why flagged:</strong> ${esc(signalReason(i))}</p><div class="details-grid"><section><h3>Callers</h3>${linkedList(i.callers)}</section><section><h3>Callees</h3>${linkedList(i.callees)}</section><section><h3>Direct internal helpers</h3>${linkedList(i.direct_internal_helpers || [])}</section><section><h3>Metrics</h3><p><strong>Depth:</strong> ${esc(i.deepest_call_chain_depth ?? '—')}</p><p><strong>Repeated helpers:</strong> ${esc(i.repeated_helper_count ?? 0)}</p></section></div></div></td></tr>`:''}`; }).join(''); }
function updateCompatibilityHelp() { $('compatibilityHelp').textContent=compatibilityContext().description; }
function update() { syncTreeActive(); renderTreeSummary(); renderTable(); } function resetFilters() { state.search=state.type=state.kind=state.review_status=state.signal=state.consistency=''; state.activeTree=''; ['searchBox','kindFilter','typeFilter','reviewStatusFilter','signalFilter'].forEach(id=>$(id).value=''); update(); }
function applyTree(key) { state.search=''; state.type=''; state.kind=''; state.review_status=''; state.signal=''; state.consistency=''; state.activeTree=key; $('searchBox').value=''; if(key.startsWith('type:')) state.type=key.slice(5); if(key.startsWith('kind:')) state.kind=key.slice(5); if(key.startsWith('review:')) state.review_status=key.slice(7); if(key.startsWith('action:')) state.signal=key.slice(7); if(key.startsWith('consistency:')) state.consistency=key.slice(12); $('kindFilter').value=state.kind ? state.kind[0].toUpperCase()+state.kind.slice(1) : ''; $('typeFilter').value=state.type; $('reviewStatusFilter').value=state.review_status; $('signalFilter').value=state.signal; update(); }
$('searchBox').addEventListener('input', e=>{state.search=e.target.value; update();});
$('kindFilter').addEventListener('change', e=>{state.kind=e.target.value.toLowerCase(); state.activeTree=''; update();}); $('typeFilter').addEventListener('change', e=>{state.type=e.target.value; state.activeTree=''; update();}); $('reviewStatusFilter').addEventListener('change', e=>{state.review_status=e.target.value; state.activeTree=''; update();}); $('compatibilityMode').addEventListener('change', e=>{state.compatibility_mode=e.target.value; updateCompatibilityHelp();}); $('selectVisible').addEventListener('click', ()=>{filteredRows().forEach(i=>state.selected.add(i.qualified_name)); renderTable();}); $('clearSelected').addEventListener('click', ()=>{state.selected.clear(); renderTable(); $('exportStatus').textContent='Selection cleared.';}); $('copyJson').addEventListener('click', ()=>copyExport('json').catch(e=>{$('exportStatus').textContent=`Unable to copy JSON: ${e.message}`;})); $('copyMarkdown').addEventListener('click', ()=>copyExport('markdown').catch(e=>{$('exportStatus').textContent=`Unable to copy Markdown: ${e.message}`;})); $('downloadJson').addEventListener('click', downloadJson); $('selectAllVisible').addEventListener('change', e=>{const rows=filteredRows(); rows.forEach(i=>e.target.checked?state.selected.add(i.qualified_name):state.selected.delete(i.qualified_name)); renderTable();}); $('signalFilter').addEventListener('change', e=>{state.signal=e.target.value; state.activeTree=''; update();}); $('resetFilters').addEventListener('click', resetFilters);
