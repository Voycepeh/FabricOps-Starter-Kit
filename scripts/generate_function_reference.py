...
ROLE_TAGS_BY_NAME = {
    ...
    "_render_data_agreement_widget_workflow": ["internal_workflow", "data_agreement_widget_workflow"],
    "_render_data_steward_widget_workflow": ["internal_workflow", "data_steward_widget_workflow"],
    ...
    "_render_maintenance_widget_workflow": ["internal_adapter", "widget_rendering_adapter"],
...
function renderCountSection(title, rows) { return `<details class="legend"><summary>${esc(title)}</summary><ul>${rows.join('')}</ul></details>`; }
function renderTreeSummary() {
  ...
  const health=[['role:unknown','Unknown role', rolePredicate('unknown')], ['role:unreachable_candidate','Unreachable candidate', rolePredicate('unreachable_candidate')], ['reach:classification_pending','Classification pending', i=>i.reachability_kind==='classification_pending' || i.review_status==='classification_pending'], ['signal:allowed_internal_role_call','Allowed internal role calls', i=>(i.architecture_signals || []).includes('allowed_internal_role_call')], ['signal:internal_calls_internal','Legacy internal_calls_internal', i=>(i.architecture_signals || []).includes('internal_calls_internal')]];
  const grouped=(title, rows)=>`<h3>${esc(title)}</h3><ul>${rows.map(([key,label,pred])=>treeButton(key,label,countBy(pred))).join('')}</ul>`;
  const layers=unique([...Object.keys(summary.function_type || {}), ...inventory.map(i=>i.function_type)]).map(label=>treeButton(`type:${label}`, label, summaryCount('function_type', label, i=>i.function_type===label)));
  const kinds=unique([...Object.keys(summary.callable_kind || {}), ...inventory.map(i=>i.callable_kind)]).map(label=>treeButton(`kind:${label}`, text(label).replace(/^./, c=>c.toUpperCase()), summaryCount('callable_kind', label, i=>i.callable_kind===label)));
  const consistency=Object.entries(layerConsistencyLabels).filter(([key])=>(summary.layer_consistency?.[key] ?? countBy(i=>i.layer_consistency===key))>0).map(([key,label])=>treeButton(`consistency:${key}`, label, summaryCount('layer_consistency', key, i=>i.layer_consistency===key)));
  const raw=unique([...inventory.flatMap(i=>i.architecture_signals || []), ...inventory.flatMap(i=>i.review_signals || []), ...inventory.flatMap(i=>i.signals || [])]).map(sig=>treeButton(`signal:${sig}`, reasonLabels[sig] || sig, countSignal(sig)));
  $('summaryTree').innerHTML=`<h2>Action-oriented summary</h2><ul>${treeButton('total','Total callables', inventory.length || summary.total_callables || summary.total_functions || 0)}</ul>${grouped('Needs action', needs)}${grouped('Protect / keep stable', protect)}${grouped('Classification health', health)}${renderCountSection('Layer counts', layers)}${renderCountSection('Callable kind counts', kinds)}${renderCountSection('Layer consistency counts', consistency)}${renderCountSection('Raw signal counts', raw)}`;
}
