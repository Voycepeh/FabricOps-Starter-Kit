...
function exportFilters() { return {quick:labelForQuick(state.quick), search:state.search, scope:state.scope, function_type:state.type, module:state.module, recommended_action:state.signal, priority:state.priority}; }
...
function refactorContext(functions) { return {selected_function_types:unique(functions.map(fn=>fn.recommended_action)), refactor_intent:'Plan safe cleanup for selected FabricOps helper functions.', refactor_mode:'planning_only', likely_refactor_actions:['Inline thin wrappers where readability improves','Merge adjacent helpers with overlapping responsibility','Keep helpers separate when naming, validation boundaries, or tests justify the abstraction'], safety_constraints:['Do not remove public API compatibility','Do not change notebook-facing behavior without calling it out','Preserve existing source paths unless a move is explicitly justified','Identify caller impact before recommending code changes','Call out tests that should be added or updated before implementation'], expected_ai_output:['Group selected functions by refactor type','Explain which functions are safe cleanup candidates','Identify functions that should not be refactored yet','Propose an ordered refactor plan','List risks and required tests','Do not produce code changes unless explicitly requested']}; }
