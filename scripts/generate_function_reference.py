...
def _classify_two_layer_edge(
    *,
    ...
    if base["result"] == "Violation":
        finding_type = "Shared helper calls public callable" if base["violation_type"] == "Internal -> Public" else base["violation_type"]
        return {"result": "Violation", "violation_type": finding_type}
    ...
    if callee_is_private and not callee_is_local_private:
        if parent_is_internal:
            return {"result": "Violation", "violation_type": "Shared helper calls private implementation"}
        return {"result": "Violation", "violation_type": "Cross-callable private dependency"}
    if callee_is_private and public_user_count > 1:
        return {"result": "Violation", "violation_type": "Cross-callable private dependency"}
    if callee_is_internal and public_user_count <= 1:
        return {"result": "Violation", "violation_type": "Single-use shared helper"}
    if parent_is_internal and callee_type == PRIVATE_HELPER_LABEL:
        return {"result": "Violation", "violation_type": "Shared helper calls private implementation"}
