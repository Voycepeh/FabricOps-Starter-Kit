...
def _build_function_inventory(
    ...
    for qn in sorted(node_by_qn, key=lambda item: (node_by_qn[item]["module_name"], node_by_qn[item]["callable_name"].lower(), item)):
        ...
        if architecture_signals:
            ...
            recommended_action = "Shared utility" if len(inbound) > 1 else "Merge candidate"
...
def _build_callable_inventory_metrics(
    ...
    callable_kind_counts = summary_counts.get("callable_kind", {})
    ...
    function_callables = int(callable_kind_counts.get("function", 0))
    ...
    return {
        ...
        "supporting_functions": max(function_callables - public_api_entrypoints, 0),
        "non_function_records": max(total_callables - function_callables, 0),
    }
...
ARCHITECTURE_VIOLATION_MEANINGS = {
    "Internal -> Public": "Internal helper is calling a notebook-facing public API.",
    "Utility -> Public": "Shared utility is calling a notebook-facing public API.",
    "Utility -> Internal": "Shared utility depends on implementation-specific internal logic.",
}
...
def _architecture_layer(row: dict[str, Any]) -> str:
    """Return the simplified Public/Internal/Utility architecture layer."""
...
def _classify_architecture_edge(caller_type: str, callee_type: str) -> dict[str, str]:
    ...
    if direction in ARCHITECTURE_VIOLATION_MEANINGS:
        return {"result": "Violation", "violation_type": direction}
    if direction == "Public -> Public":
        return {"result": "Review", "violation_type": ""}
        ...
        def callee_row(qn: str, edge_type: str) -> dict[str, Any]:
            ...
            return {
                ...
                **({"docs_path": row["docs_path"], "docs_url": row["docs_url"]} if row.get("docs_url") else {}),
                ...
                "helper_cleanup_candidate": _decision_layer_group(row) != "Public API"
...
def _build_callable_flow_data(
    ...
    summary_counts["public_api_surface"] = {
        ...
        "architecture_violations": sum(flow["architecture_violation_count"] for flow in public_entrypoint_flow),
        "boundary_violations": sum(flow["architecture_violation_count"] for flow in public_entrypoint_flow),
        "architecture_findings": sum(len(flow.get("warnings", [])) for flow in public_entrypoint_flow),
        "cross_layer_issues": sum(flow["architecture_violation_count"] for flow in public_entrypoint_flow),
        "merge_candidates": sum(flow["single_use_helper_candidate_count"] for flow in public_entrypoint_flow),
...
def _render_refactor_inventory_html(flow_data: dict[str, Any]) -> str:
    ...
    del flow_data
    return r"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Callable Inventory</title><style>body{margin:0;font-family:Inter,system-ui,sans-serif;background:#f8fafc;color:#0f172a}header,main{max-width:1500px;margin:0 auto;padding:1rem}.surface-cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:.55rem;margin:.75rem 0}.surface-card{background:#fff;border:1px solid #dbe3ef;border-radius:.75rem;padding:.65rem}.surface-card strong{display:block;margin-bottom:.25rem;line-height:1;font-size:1.45rem}.surface-card span{display:block;line-height:1.2;font-weight:700}.surface-card small{display:block;margin-top:.3rem;line-height:1.25;color:#64748b}.surface-card.info{border-color:#bfdbfe;background:#eff6ff}.surface-card.primary{border-color:#c7d2fe;background:#eef2ff}.surface-card.secondary{background:#f8fafc}.surface-card.muted{background:#f1f5f9;color:#475569}.surface-card.info strong,.surface-card.primary strong{color:#1d4ed8}.surface-card.muted strong{color:#475569}.filter-panel{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:.55rem}.filter-field{display:flex;flex-direction:column;font-weight:700;font-size:.82rem}input,select,button{padding:.42rem .5rem;border:1px solid #dbe3ef;border-radius:.45rem;background:#fff}.export-toolbar{display:flex;flex-direction:column;gap:.65rem;align-items:stretch;box-sizing:border-box;max-width:100%;margin:.75rem 0}.toolbar-card-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,18rem),1fr));gap:.65rem}.toolbar-card{box-sizing:border-box;min-width:0;padding:.65rem .75rem;border:1px solid #dbe3ef;border-radius:.75rem;background:#f1f5f9}.toolbar-card--selection,.toolbar-card--prompt{display:flex;flex-direction:column;gap:.55rem}.toolbar-row{display:flex;flex-wrap:wrap;gap:.5rem;align-items:center;max-width:100%;min-width:0}.toolbar-field{display:flex;flex-wrap:wrap;gap:.35rem;align-items:center;max-width:100%;min-width:0;font-weight:700}.toolbar-field select{max-width:100%;min-width:0}.toolbar-help,.group-help{color:#475569;font-weight:400}.toolbar-group{display:flex;flex-wrap:wrap;gap:.5rem 1rem;align-items:center;max-width:100%;min-width:0}.toolbar-group>div{min-width:0}.toolbar-group .toolbar-row{flex:1 1 16rem}.toolbar-row button{white-space:normal}.primary-action,.danger-action:not(:disabled){background:#1d4ed8;color:#fff;border-color:#1d4ed8}.danger-action:not(:disabled){background:#b91c1c;border-color:#b91c1c}button:disabled,.danger-action:disabled{background:#e5e7eb;color:#94a3b8;border-color:#cbd5e1;cursor:not-allowed}.table-wrap{overflow:auto;background:#fff;border:1px solid #dbe3ef}table{width:100%;min-width:1500px;border-collapse:collapse}th,td{padding:.42rem .45rem;border-bottom:1px solid #dbe3ef;text-align:left;font-size:.84rem;vertical-align:top}.tag,.badge{display:inline-flex;align-items:center;margin:.06rem;padding:.12rem .42rem;border-radius:999px;background:#e0e7ff;color:#3730a3;font-size:.69rem;font-weight:800;line-height:1.2}.badge.issue{background:#fee2e2;color:#991b1b}.badge.warn{background:#fef3c7;color:#92400e}.badge.keep{background:#dcfce7;color:#166534}.badge.muted{background:#e2e8f0;color:#475569}.priority-high{background:#fee2e2;color:#991b1b}.priority-medium,.priority-review{background:#fef3c7;color:#92400e}.priority-low{background:#e0f2fe;color:#075985}.priority-protect{background:#dcfce7;color:#166534}.priority-help ul{margin:.35rem 0 .15rem 1.1rem;padding:0}.priority-help li{margin:.12rem 0}.manual-copy{width:100%;min-height:8rem}@media(max-width:720px){main,header{padding:.75rem}.toolbar-card-grid{grid-template-columns:1fr}.toolbar-row,.toolbar-group{align-items:stretch}.toolbar-field,.toolbar-group{flex-direction:column}.toolbar-field select{width:100%}.toolbar-help{display:block}.toolbar-group .toolbar-row{flex:1 1 auto;flex-direction:row}.toolbar-group .toolbar-row button{flex:1 1 9rem}}</style></head><body><header><h1>Callable Inventory</h1><p>Search/filter callables with maintainer-friendly role groups, reachability, and refactor signals.</p><p><a href="callable-functions-dashboard.html">Architecture</a> - <a href="../reference/_data/callable-flow.json">Open JSON data</a></p></header><main><section id="inventorySummaryCards" class="surface-cards" aria-label="Callable inventory summary"></section><p><small>Callable metrics are generated from the callable inventory data.</small></p><section class="filter-panel" aria-label="Primary inventory filters"><label class="filter-field">Callable <input id="searchBox" type="search"></label><label class="filter-field">Module <select id="moduleFilter"><option value="">All modules</option></select></label><label class="filter-field">Role group <select id="roleGroupFilter"><option value="">All role groups</option></select></label><label class="filter-field">Reachability <select id="reachabilityFilter"><option value="">All reachability</option></select></label><label class="filter-field">Signal <select id="signalFilter"><option value="">All signals</option></select></label><label class="filter-field">Priority <select id="priorityFilter"><option value="">All priorities</option></select></label><button id="resetFilters" type="button">Reset filters</button></section><details class="advanced-filters"><summary>Advanced / Debug filters</summary><section class="filter-panel"><label class="filter-field">Role detail <select id="callableRoleFilter"><option value="">All role details</option></select></label><label class="filter-field">Dependency role <select id="dependencyRoleFilter"><option value="">All dependency roles</option></select></label><label class="filter-field">Callable kind <select id="kindFilter"><option value="">All kinds</option></select></label><label class="filter-field">Layer <select id="typeFilter"><option value="">All layers</option></select></label><label class="filter-field">Review status <select id="reviewStatusFilter"><option value="">All review statuses</option></select></label><label class="filter-field">Min inbound <input id="minInboundFilter" type="number" min="0" value="0"></label><label class="filter-field">Min outbound <input id="minOutboundFilter" type="number" min="0" value="0"></label></section></details><section class="toolbar-card priority-help" aria-label="Priority explanation"><strong>Priority guide</strong><p>Priority is generated from callable inventory signals, architecture findings, review status, risk, and suggested action. It is a triage aid, not an automatic refactor instruction.</p><ul><li><strong>High</strong> = strongest review/refactor signal; inspect first.</li><li><strong>Medium</strong> = useful cleanup candidate; review before changing.</li><li><strong>Low</strong> = minor or low-risk cleanup.</li><li><strong>Review</strong> = human judgement required.</li><li><strong>Protect</strong> = stable/shared/public surface; do not casually refactor.</li></ul></section><section class="export-toolbar"><div class="toolbar-card-grid"><div class="toolbar-card toolbar-card--selection" aria-label="Selection"><strong id="selectedCount">Selected: 0 callables</strong><div class="toolbar-row"><button id="selectVisible" class="primary-action" type="button">Select visible</button><button id="clearSelected" class="danger-action" type="button" disabled>Clear selection</button></div></div><div class="toolbar-card toolbar-card--compatibility"><label class="toolbar-field">Compatibility mode <select id="compatibilityMode"><option value="internal_cleanup" selected>Internal cleanup</option><option value="stable_api_safe">Stable API safe</option><option value="development_breaking_allowed">Development breaking allowed</option></select><small class="toolbar-help">Sets the API safety assumptions for generated refactor packages.</small></label></div></div><div class="toolbar-card toolbar-card--prompt"><div class="toolbar-group"><div><strong>AI prompt</strong><br><small class="group-help">Export selected callables for AI-assisted refactor planning.</small></div><div class="toolbar-row"><button id="copyJson" type="button" disabled>Copy JSON</button><button id="copyMarkdown" type="button" disabled>Copy Markdown</button><button id="downloadJson" type="button" disabled>Download JSON</button></div></div><small id="exportStatus"></small><textarea id="manualCopy" class="manual-copy" hidden readonly></textarea></div></section><p id="resultCount"></p><section class="table-wrap"><table><thead><tr><th>Select<br><input id="selectAllVisible" type="checkbox"></th><th>Callable</th><th>Module</th><th>Role group</th><th>Role detail</th><th>Reachability</th><th>Findings / Signal</th><th>Suggested action</th><th>Priority</th><th>Inbound</th><th>Outbound</th><th>Dependency role</th><th>Kind / Layer</th></tr></thead><tbody id="inventoryBody"></tbody></table></section></main><script>
const DISPLAY_LABEL_MAP={'Architecture violation':'Architecture violation','Long call chain':'Long call chain','Large dependency surface':'Large dependency surface','Merge candidate':'Merge candidate','Review abstraction':'Review abstraction','Shared utility':'Shared utility'};const USER_FACING_RECOMMENDED_ACTIONS=['Architecture violation','Merge candidate','Review abstraction','Review manually','Shared utility','Shared internal helper','Orphaned callable','Unreachable candidate'];const ROLE_GROUP_FILTER_OPTIONS=[['public_entrypoint','Public entrypoint'],['workflow','Workflow'],['resolver','Resolver'],['normalizer','Normalizer'],['validator','Validator'],['adapter','Adapter'],['utility','Utility'],['model_class','Model class'],['registry_builder','Registry builder'],['lifecycle_method','Lifecycle method'],['property_method','Property method'],['other','Other']];let inventory=[],visibleRows=[],summaryCounts={};const state={search:'',kind:'',type:'',review_status:'',module:'',signal:'',priority:'',callable_role:'',callable_role_group:'',dependency_role:'',reachability_kind:'',min_inbound:0,min_outbound:0,selected:new Set()};const $=id=>document.getElementById(id),text=v=>String(v??''),esc=v=>text(v).replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));const COMPATIBILITY_INSTRUCTIONS={internal_cleanup:'Preserve public API behavior. Internal helpers may be renamed, moved, privatized, inlined, or simplified when tests and callers remain valid.',stable_api_safe:'Treat public behavior and documented contracts as stable. Prefer additive or internal-only changes and avoid breaking notebook-facing APIs.',development_breaking_allowed:'Breaking changes are allowed only when they clearly simplify the design; update callers, tests, and documentation in the same change.'};function selectedItems(){return inventory.filter(row=>state.selected.has(row.qualified_name))}function exportItem(r){return{qualified_name:r.qualified_name,function_name:r.function_name,module:r.module,layer:r.layer||r.function_type,function_type:r.function_type,callable_kind:r.callable_kind,recommended_action:r.recommended_action,priority:r.priority||r.review_status_label,callable_role:r.callable_role_detail,dependency_role:r.dependency_role,reachability_kind:r.reachability_kind,callers_count:r.called_by_count,callees_count:r.calls_count,source_url:r.source_url,signals:r.signals,callers:r.callers,callees:r.callees}}function refactorContext(callables){const mode=$('compatibilityMode').value;return{compatibility_mode:mode,compatibility_instruction:COMPATIBILITY_INSTRUCTIONS[mode]}}function batchAccounting(callables){return{selected_callable_count:callables.length,planned_batch_count:callables.length,completed_or_refactored_count:'fill in after implementation',remaining_selected_count:'fill in after implementation',deferred_selected_callables:'fill in after implementation'}}function refactorPacket(){const callables=selectedItems().map(exportItem);return{schema:'fabricops_callable_refactor_packet',callables,functions:callables,batch_accounting:batchAccounting(callables),refactor_context:refactorContext(callables),ai_prompt:'Review selected callables'}}function mdField(label,value){const v=Array.isArray(value)?value.join(', '):(value??'');return `- ${label}: ${v||'None'}`}function callableMarkdown(item){return [`### ${item.qualified_name}`,mdField('qualified_name',item.qualified_name),mdField('function_name',item.function_name),mdField('module',item.module),mdField('layer / function_type',item.layer||item.function_type),mdField('callable_kind',item.callable_kind),mdField('recommended_action',item.recommended_action),mdField('priority or risk',item.priority),mdField('callable_role',item.callable_role),mdField('dependency_role',item.dependency_role),mdField('reachability_kind',item.reachability_kind),mdField('callers count',item.callers_count),mdField('callees count',item.callees_count),mdField('source_url',item.source_url),mdField('signal reason / signals',item.signal_reason||item.signals)].join('\n')}function markdownPacket(packet){return `# Callable refactor packet\n\n## Objective\nReview and refactor the selected FabricOps callables to improve maintainability, simplify long call chains, review architecture findings, and preserve expected public behavior.\n\n## Architecture intent\n- Keep the public API surface small and clear.\n- Public API callables may be heavier if that makes the code easier to follow.\n- Prefer public -> internal workflow/resolver/validator -> utility/model flow.\n- Avoid long helper chains.\n- Merge small helper into parent when readability improves.\n- Move helpers closer to their caller when they are not genuinely reusable.\n- Keep genuinely shared utilities stable and generic.\n- Do not casually change public API behavior.\n\n## Compatibility mode\n- selected mode: ${packet.refactor_context.compatibility_mode}\n- instruction: ${packet.refactor_context.compatibility_instruction}\n\n## Selected callables\n${packet.callables.map(callableMarkdown).join('\n\n')}\n\n## Requested work\nFor each selected callable:\n1. Inspect the callable and its direct/downstream dependencies.\n2. Decide whether to keep public, flatten internals, merge candidates, move helper closer to caller, or review architecture findings.\n3. Preserve external behavior unless compatibility mode explicitly allows breaking changes.\n4. Prefer simpler, flatter code over long helper chains.\n5. Keep utilities generic and prevent utility -> project-specific workflow dependencies.\n6. Update or add tests where needed.\n7. Avoid broad unrelated refactors.\n\n## Output required from AI\nReturn:\n- Summary of changes\n- Callables changed\n- Callables intentionally left unchanged\n- Tests added/updated\n- Risks\n- Remaining follow-up items\n- Batch accounting: completed, deferred, remaining\n\n## Batch accounting\n- selected_callable_count: ${packet.batch_accounting.selected_callable_count}\n- planned_batch_count: ${packet.batch_accounting.planned_batch_count}\n- completed_or_refactored_count: fill in after implementation\n- remaining_selected_count: fill in after implementation\n- deferred_selected_callables: fill in after implementation\n`;}async function copyExport(format){const packet=refactorPacket();const value=format==='markdown'?markdownPacket(packet):JSON.stringify(packet,null,2);try{await navigator.clipboard.writeText(value);$('exportStatus').textContent=`Copied ${format}.`;$('manualCopy').hidden=true}catch(e){$('manualCopy').hidden=false;$('manualCopy').value=value;$('exportStatus').textContent='Clipboard unavailable; copy from the text area.'}}function downloadJson(){const blob=new Blob([JSON.stringify(refactorPacket(),null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='fabricops-callable-refactor-packet.json';a.click()}function unique(v){return[...new Set(v.filter(Boolean))].sort()}function displayLabel(v){return DISPLAY_LABEL_MAP[text(v)]||text(v)}function label(v){return displayLabel(text(v).replace(/_/g,' ').replace(/\b\w/g,ch=>ch.toUpperCase()))}function option(s,v,l){const o=document.createElement('option');o.value=v;o.textContent=l||v;s.appendChild(o)}const GITHUB_SOURCE_BASE='https://github.com/Voycepeh/FabricOps-Starter-Kit/blob/main/';function sourceHref(i){if(i.docs_path)return '../'+String(i.docs_path).replace(/^docs\//,'').replace(/\.md$/,'/');if(i.source_url)return i.source_url;const path=i.source_path;if(!path)return '';const start=i.source_start_line;const end=i.source_end_line;let anchor=start?`#L${start}`:'';if(start&&end&&end!==start)anchor+=`-L${end}`;return GITHUB_SOURCE_BASE+path+anchor}function sourceCallableLink(i){const href=sourceHref(i);const label=esc(i.function_name);return href?`<a class="source-link" href="${esc(href)}" target="_blank" rel="noopener"><code>${label}</code></a>`:`<code>${label}</code>`}function inventoryMetrics(){return summaryCounts.callable_inventory_metrics||{module_count:0,total_callables:0,public_api_entrypoints:0,supporting_functions:0,non_function_records:0}}function renderInventoryCards(){const m=inventoryMetrics(),cards=[{label:'Modules',value:m.module_count,subtitle:'Code areas represented.',cls:'secondary'},{label:'Total callables',value:m.total_callables,subtitle:'Complete discovered callable inventory.',cls:'info'},{label:'Public API',value:m.public_api_entrypoints,subtitle:'Notebook-facing entrypoints.',cls:'primary'},{label:'Supporting functions',value:m.supporting_functions,subtitle:'Internal functions behind the public API.',cls:'secondary'},{label:'Non-function records',value:m.non_function_records,subtitle:'Classes, aliases, or metadata records.',cls:'muted'}];$('inventorySummaryCards').innerHTML=cards.map(c=>`<article class="surface-card ${esc(c.cls)}"><strong>${esc(c.value??0)}</strong><span>${esc(c.label)}</span><small>${esc(c.subtitle)}</small></article>`).join('')}function populateFilters(){unique(inventory.map(i=>i.callable_kind)).forEach(v=>option($('kindFilter'),v,label(v)));['Public API','Internal helper','Utility'].forEach(v=>option($('typeFilter'),v));unique(inventory.map(i=>i.review_status_label)).forEach(v=>option($('reviewStatusFilter'),v));unique(inventory.map(i=>i.module)).forEach(v=>option($('moduleFilter'),v));ROLE_GROUP_FILTER_OPTIONS.forEach(([v,l])=>option($('roleGroupFilter'),v,l));unique(inventory.map(i=>i.reachability_kind)).forEach(v=>option($('reachabilityFilter'),v,(inventory.find(i=>i.reachability_kind===v)||{}).reachability_label||label(v)));const present=new Set(inventory.map(i=>displayLabel(i.recommended_action)));USER_FACING_RECOMMENDED_ACTIONS.filter(v=>present.has(v)).forEach(v=>option($('signalFilter'),v));unique(inventory.map(i=>i.priority)).forEach(v=>option($('priorityFilter'),v));unique(inventory.flatMap(i=>i.callable_role_detail||i.callable_role||[])).forEach(v=>option($('callableRoleFilter'),v,label(v)));unique(inventory.map(i=>i.dependency_role)).forEach(v=>option($('dependencyRoleFilter'),v,label(v)))}function filteredRows(){const q=state.search.toLowerCase();return inventory.filter(i=>(!q||text(i.function_name).toLowerCase().includes(q)||text(i.qualified_name).toLowerCase().includes(q))&&(!state.kind||i.callable_kind===state.kind)&&(!state.type||i.function_type===state.type)&&(!state.review_status||i.review_status_label===state.review_status)&&(!state.module||i.module===state.module)&&(!state.signal||displayLabel(i.recommended_action)===state.signal)&&(!state.priority||(i.priority)===state.priority)&&(!state.callable_role_group||i.callable_role_group===state.callable_role_group)&&(!state.callable_role||(i.callable_role_detail||i.callable_role||[]).includes(state.callable_role))&&(!state.dependency_role||i.dependency_role===state.dependency_role)&&(!state.reachability_kind||i.reachability_kind===state.reachability_kind)&&(i.called_by_count)>=state.min_inbound&&(i.calls_count??0)>=state.min_outbound)}function updateExportControls(){$('selectedCount').textContent=`Selected: ${state.selected.size} callables`;['copyJson','copyMarkdown','downloadJson'].forEach(id=>$(id).disabled=state.selected.size===0);$('clearSelected').disabled=state.selected.size===0;if($('selectAllVisible'))$('selectAllVisible').checked=visibleRows.length>0&&visibleRows.every(r=>state.selected.has(r.qualified_name))}function resultCountText(){const total=summaryCounts.total_callables??inventory.length;return `Showing ${visibleRows.length} callable records of ${total} total discovered callable records.`}function signalBadgeClass(action){const a=displayLabel(action);if(a==='Architecture violation')return 'issue';if(a==='Merge candidate'||a==='Review abstraction'||a==='Review manually')return 'warn';if(a==='Shared utility'||a==='Shared internal helper'||a==='Protect public API')return 'keep';return 'muted'}function priorityClass(priority){const p=text(priority).toLowerCase();if(p.includes('high'))return 'high';if(p.includes('medium'))return 'medium';if(p.includes('low'))return 'low';if(p.includes('protect'))return 'protect';if(p.includes('review'))return 'review';return 'muted'}function priorityBadge(i){const p=i.priority||'Review';return `<span class="badge priority-${esc(priorityClass(p))}">${esc(p)}</span>`}function roleGroupCell(i){const detail=(i.callable_role_detail_label||label((i.callable_role_detail||i.callable_role||[]).join(', ')));return `<span class="tag" title="${esc(detail)}">${esc(i.callable_role_group_label||label(i.callable_role_group))}</span>`}function roleDetailCell(i){const detail=(i.callable_role_detail_label||label((i.callable_role_detail||i.callable_role||[]).join(', ')));return detail?`<small>${esc(detail)}</small>`:'<small>None</small>'}function signalCell(i){const action=displayLabel(i.recommended_action);return `<span class="badge ${esc(signalBadgeClass(action))}">${esc(action)}</span>`}function suggestedActionCell(i){return esc(displayLabel(i.recommended_action||'Review manually'))}function kindLayerCell(i){return `<small>${esc(label(i.callable_kind))}<br>${esc(i.function_type||i.layer||'')}</small>`}function renderTable(){visibleRows=filteredRows();$('resultCount').textContent=resultCountText();$('inventoryBody').innerHTML=visibleRows.map(i=>`<tr><td><input type="checkbox" data-select-row="${esc(i.qualified_name)}" ${state.selected.has(i.qualified_name)?'checked':''}></td><td>${sourceCallableLink(i)}</td><td><code>${esc(i.module)}</code></td><td>${roleGroupCell(i)}</td><td>${roleDetailCell(i)}</td><td>${esc(i.reachability_label||label(i.reachability_kind))}</td><td>${signalCell(i)}</td><td>${suggestedActionCell(i)}</td><td>${priorityBadge(i)}</td><td>${esc(i.called_by_count)}</td><td>${esc(i.calls_count??0)}</td><td>${esc(label(i.dependency_role))}</td><td>${kindLayerCell(i)}</td></tr>`).join('');updateExportControls()}['searchBox'].forEach(id=>$(id).addEventListener('input',e=>{state.search=e.target.value;renderTable()}));[['kindFilter','kind'],['typeFilter','type'],['reviewStatusFilter','review_status'],['moduleFilter','module'],['signalFilter','signal'],['priorityFilter','priority'],['roleGroupFilter','callable_role_group'],['callableRoleFilter','callable_role'],['dependencyRoleFilter','dependency_role'],['reachabilityFilter','reachability_kind']].forEach(([id,k])=>$(id).addEventListener('change',e=>{state[k]=e.target.value;renderTable()}));[['minInboundFilter','min_inbound'],['minOutboundFilter','min_outbound']].forEach(([id,k])=>$(id).addEventListener('input',e=>{state[k]=Number(e.target.value||0);renderTable()}));$('resetFilters').onclick=()=>location.reload();document.addEventListener('change',e=>{const r=e.target.closest('[data-select-row]');if(r){r.checked?state.selected.add(r.dataset.selectRow):state.selected.delete(r.dataset.selectRow);updateExportControls()}});$('selectAllVisible').onchange=e=>{e.target.checked?visibleRows.forEach(r=>state.selected.add(r.qualified_name)):visibleRows.forEach(r=>state.selected.delete(r.qualified_name));renderTable()};$('selectVisible').onclick=()=>{visibleRows.forEach(r=>state.selected.add(r.qualified_name));renderTable()};$('clearSelected').onclick=()=>{state.selected.clear();renderTable()};$('copyJson').onclick=()=>copyExport('json');$('copyMarkdown').onclick=()=>copyExport('markdown');$('downloadJson').onclick=downloadJson;async function loadData(){const response=await fetch('../reference/_data/callable-flow.json');const data=await response.json();inventory=data.function_inventory||[];summaryCounts=data.summary_counts||{};renderInventoryCards();populateFilters();renderTable()}loadData();
</script></body></html>
"""
...
def main() -> None:
    ...
    ref = [
        ...
        '  </section>',
        '  <section class="reference-kpi-card surface-card">',
        f'    <strong class="reference-kpi-value">{callable_metrics["non_function_records"]}</strong>',
