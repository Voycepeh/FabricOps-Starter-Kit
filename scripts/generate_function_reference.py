...
def _render_dashboard_architecture_scope_rows(flow_data: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        f'<td class="num">{all_runtime_count}</td><td class="num">—</td><td></td><td><span class="badge info">Runtime scope</span></td><td></td></tr>',
        ...
        f'<td class="num">{unreachable_count}</td><td class="num">—</td><td><span class="badge warn">Verify possible orphan</span></td><td><span class="badge warn">Cannot trace back to a public function</span></td><td></td></tr>',
    ]
    if not public_flows:
        rows.append('<tr data-public-flow-empty="true"><td colspan="7">No public callable flows found in function-call-graph.json.</td></tr>')
        return "".join(rows)
    for index, flow in enumerate(public_flows):
        ...
        rows.append(
            f'<tr data-public-flow-row="{qn}" data-public-flow-search="{html.escape(" ".join([str(flow.get("function_name") or ""), str(flow.get("qualified_name") or ""), " ".join(signals)]).lower(), quote=True)}" class="{active} severity-{severity}-row" tabindex="0" aria-selected="{str(index == 0).lower()}">'
            f'<td>{link}</td><td class="num">{html.escape(str(flow.get("width", 0)))}</td><td class="num">{html.escape(str(flow.get("scope", flow.get("scope_asset_count", 0))))}</td><td class="num">{html.escape(str(flow.get("max_depth", 0)))}</td><td>{recommendation_html}</td><td>{_render_dashboard_badges(signals, signal_class)}</td><td>{summary}</td></tr>'
        )
    return "".join(rows)
