...
def _render_callable_generation_banner(flow_data: dict[str, Any]) -> str:
    ...
    data_source_generated_at = _format_generated_at_sgt(metadata.get("generated_at_utc"))
    dashboard_ui_generated_at = _format_generated_at_sgt(
        metadata.get("dashboard_ui_generated_at_utc") or metadata.get("generated_at_utc")
    )
    parts = [
        f"<strong>Data source generated at:</strong> {html.escape(data_source_generated_at)}",
        f"<span><strong>Dashboard UI generated at:</strong> {html.escape(dashboard_ui_generated_at)}</span>",
    ]
...
def _runtime_inventory_section_html(flow_data: dict[str, Any]) -> str:
    ...
    return (
        ...
        '<h2>Current scope runtime inventory</h2>'
...
def _runtime_inventory_embed_css() -> str:
    ...
    return (
        ...
        "#runtime-inventory .inventory-filter-grid{display:grid;grid-template-columns:minmax(16rem,1fr) auto;gap:.55rem;margin:.75rem 0;align-items:end}.scope-banner-row{display:flex;justify-content:space-between;gap:.75rem;align-items:center}.scope-banner-row p{margin:.25rem 0 0}.scope-current-function{display:block;margin-top:.25rem;font-weight:800;color:#1d4ed8;overflow-wrap:anywhere}.inventory-search-grid .inventory-clear-all{align-self:end}"
