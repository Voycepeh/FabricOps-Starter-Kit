...
def _flow_data_public_flows_for_dashboard(flow_data: dict[str, Any]) -> tuple[list[dict[str, Any]], bool]:
    ...
    public_flows = flow_data.get("public_flows") or flow_data.get("public_entrypoint_flow") or []
    inventory_rows = flow_data.get("function_inventory") or []
    if public_flows:
        rows = list(public_flows)
        existing_names = {str(flow.get("qualified_name") or flow.get("function_name") or "") for flow in rows}
        existing_names.update(str(flow.get("function_name") or "") for flow in rows)
        fallback_rows, _ = _flow_data_public_flows_for_dashboard({"function_inventory": inventory_rows})
        for row in fallback_rows:
            if str(row.get("qualified_name") or "") not in existing_names and str(row.get("function_name") or "") not in existing_names:
                rows.append(row)
        return rows, len(rows) > len(public_flows)
    rows = []
    for row in inventory_rows:
        classification = str(row.get("simple_classification") or row.get("function_type") or "")
        if classification == "Public function" or row.get("reachability") == "public_entrypoint":
            rows.append({
...
def _dashboard_public_flow_signals(flow: dict[str, Any]) -> list[str]:
    """Return compact architecture signals for a public flow."""
    signals: list[str] = []
    if int(flow.get("architecture_violation_count") or 0):
        signals.append("Broken rule")
    if int(flow.get("helper_cleanup_candidates") or 0):
        signals.append("Maybe combine")
    if int(flow.get("external_dependents_count") or 0):
        signals.append("Shared helper")
    return signals or ["Healthy"]


def _dashboard_public_flow_recommendation(flow: dict[str, Any]) -> str:
    """Return the static dashboard recommendation for a public flow."""
    if int(flow.get("architecture_violation_count") or 0):
        return "Fix architecture issue"
    if int(flow.get("helper_cleanup_candidates") or 0):
        return "Review helper cleanup"
    return ""



def _dashboard_architecture_summary_counts(flow_data: dict[str, Any]) -> dict[str, int]:
    """Return initial dashboard architecture summary counts from generated data."""
    ...
    def is_public(row: dict[str, Any]) -> bool:
        markers = [
            ...
            row.get("layer"),
            row.get("callable_role_group"),
            row.get("reachability"),
            row.get("reachability_kind"),
        ]
        return any(text(marker) in {"public function", "public", "public_entrypoint", "public api entrypoint", "public_api_entrypoint"} for marker in markers)

    def is_private(row: dict[str, Any]) -> bool:
        markers = [row.get("function_type"), row.get("layer"), row.get("callable_role_group")]
        return any("private" in text(marker) for marker in markers)

    def is_shared(row: dict[str, Any]) -> bool:
        if is_public(row) or is_private(row):
            return False
        markers = [row.get("function_type"), row.get("layer"), row.get("callable_role_group"), row.get("usage_scope")]
        return any(text(marker) in {"shared helper", "internal", "shared", "shared_runtime"} for marker in markers)

    def needs_review(row: dict[str, Any]) -> bool:
        values = [row.get("recommended_action"), row.get("review_status"), row.get("priority"), *(row.get("signals") or [])]
        return any(value and text(value) not in {"healthy", "no action", "low", "none"} for value in values)

    def has_issue(row: dict[str, Any]) -> bool:
        values = [row.get("architecture_result"), row.get("violation_type"), row.get("recommended_action"), *(row.get("signals") or []), *(row.get("architecture_signals") or [])]
        return any("violation" in text(value) or "broken rule" in text(value) or "architecture" in text(value) for value in values)

    return {
        "public": sum(1 for row in rows if is_public(row)),
        "shared": sum(1 for row in rows if is_shared(row)),
        "private": sum(1 for row in rows if is_private(row)),
        "review": sum(1 for row in rows if needs_review(row)),
        "issues": sum(1 for row in rows if has_issue(row)),
    }

def _render_dashboard_badges(labels: list[str], css_class: str = "info") -> str:
    """Render static dashboard badges."""
    return "".join(f'<span class="badge {css_class}">{html.escape(str(label))}</span>' for label in labels[:3])
...
def _render_dashboard_architecture_scope_rows(flow_data: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        f'<td class="num">{all_runtime_count}</td><td class="num">—</td><td></td><td><span class="badge info">Runtime scope</span></td></tr>',
    ...
    for index, flow in enumerate(public_flows):
        ...
        link = (
            f'<button type="button" class="callable-button" data-public-flow-select="{qn}"><code>{name}</code></button> '
            f'<a class="source-link" href="{html.escape(str(href), quote=True)}" aria-label="Open docs or source for {name}">↗</a>'
            if href
            else f'<button type="button" class="callable-button" data-public-flow-select="{qn}"><code>{name}</code></button>'
        )
        recommendation = _dashboard_public_flow_recommendation(flow)
        recommendation_html = f'<span class="badge recommendation-chip {"issue" if "architecture" in recommendation.lower() else "warn"}">{html.escape(recommendation)}</span>' if recommendation else ""
        signals = _dashboard_public_flow_signals(flow)
        signal_class = "issue" if "Broken rule" in signals else "warn" if recommendation else "info"
        severity = "architecture" if "Broken rule" in signals else "neutral"
        active = " active" if index == 0 else ""
        source_note = " Inventory row fallback." if from_inventory else ""
        summary = html.escape(("Detailed call flow was not available for this public function." if flow.get("flow_source") == "function_inventory" else "Public callable architecture scope rendered at generation time.") + source_note)
        rows.append(
            f'<tr data-public-flow-row="{qn}" data-public-flow-summary="{summary}" data-public-flow-search="{html.escape(" ".join([str(flow.get("function_name") or ""), str(flow.get("qualified_name") or ""), " ".join(signals)]).lower(), quote=True)}" class="{active} severity-{severity}-row" tabindex="0" aria-selected="{str(index == 0).lower()}">'
            f'<td>{link}</td><td class="num">{html.escape(str(flow.get("width", 0)))}</td><td class="num">{html.escape(str(flow.get("scope", flow.get("scope_asset_count", 0))))}</td><td class="num">{html.escape(str(flow.get("max_depth", 0)))}</td><td>{recommendation_html}</td><td>{_render_dashboard_badges(signals, signal_class)}</td></tr>'
        )
    return "".join(rows)
...
def _runtime_inventory_embed_css() -> str:
    ...
    return (
        ...
        "#runtime-inventory .callable-review-table tr.selected{outline:1px solid #bfdbfe;outline-offset:-1px;background:#f8fbff;box-shadow:inset .18rem 0 #93c5fd}.callable-review-table tr.scope-highlight{outline:2px solid #15803d;outline-offset:-2px;background:#f0fdf4;box-shadow:inset .25rem 0 #15803d}"
