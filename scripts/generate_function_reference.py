...
def _flow_data_public_flows_for_dashboard(flow_data: dict[str, Any]) -> tuple[list[dict[str, Any]], bool]:
    ...
    for row in inventory_rows:
        ...
        if (
            classification in {"Public function", "Public config class", "Class"}
            or (callable_kind == "class" and layer == "public")
            or row.get("reachability") == "public_entrypoint"
        ):
            rows.append({
...
def _runtime_inventory_section_html(flow_data: dict[str, Any]) -> str:
    ...
    return (
        ...
        '<h2>Current scoped callables inventory</h2>'
