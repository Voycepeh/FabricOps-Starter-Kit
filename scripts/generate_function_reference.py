...
def _dashboard_architecture_summary_counts(flow_data: dict[str, Any]) -> dict[str, int]:
    ...
    signals = sum(1 for flow in public_flows if _dashboard_public_flow_signals(flow) != ["No signal"])
    return {"public": public_count, "shared": shared_count, "private": private_count, "review": review, "issues": issues, "signals": signals}
...
def _render_dashboard_architecture_scope_rows(flow_data: dict[str, Any]) -> str:
    ...
    for index, flow in enumerate(public_flows):
        ...
        rows.append(
            ...
            f'<td>{link}</td><td class="num">{html.escape(str(flow.get("width", 0)))}</td><td class="num">{html.escape(str(flow.get("scope", flow.get("scope_asset_count", 0))))}</td><td class="num">{html.escape(str(flow.get("max_depth", 0)))}</td><td>{_render_dashboard_badges(signals, signal_class)}</td></tr>'
        )
    return "".join(rows)
