...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .legend { margin:.75rem 0 1rem; } .legend summary { cursor:pointer; font-weight:700; color:#334155; } .legend-grid { display:grid; grid-template-columns: repeat(auto-fit, minmax(230px, 1fr)); gap:.5rem; margin-top:.6rem; } .legend-grid div { border:1px solid var(--border); border-radius:.7rem; padding:.65rem; background:#fff; }
    ...
    th:nth-child(1) { width:5rem; } th:nth-child(2) { width:18rem; } th:nth-child(3) { width:8rem; } th:nth-child(4) { width:9rem; } th:nth-child(5) { width:16rem; } th:nth-child(6), th:nth-child(7), th:nth-child(8) { width:6rem; } th:nth-child(9) { width:15rem; }
...
<main>
  ...
  <p id="resultCount"></p>
  <section class="table-wrap" aria-label="Combined function inventory"><table><thead><tr><th></th><th><button type="button" data-sort="function_name">Function</button></th><th><button type="button" data-sort="function_type">Function type</button></th><th><button type="button" data-sort="module">Module</button></th><th>Signal / shape</th><th>Recommended action<br><small>Priority</small></th><th class="num"><button type="button" data-sort="called_by_count">Called by</button></th><th class="num"><button type="button" data-sort="calls_count">Calls</button></th><th>Source</th></tr></thead><tbody id="inventoryBody"></tbody></table></section>
</main>
<script>
...
function renderTable() { const rows=filteredRows().sort(compare); $('resultCount').textContent=`Showing ${rows.length} of ${inventory.length} discovered functions.`; $('inventoryBody').innerHTML=rows.map(i=>{ const id=i.qualified_name, open=state.expanded.has(id), p=text(i.priority).toLowerCase(), t=text(i.function_type).toLowerCase(); return `<tr><td><button type="button" class="action-cell" data-toggle="${esc(id)}" aria-expanded="${open}">${open?'Hide':'View'}</button></td><td class="function-name" title="${esc(i.qualified_name)}"><a href="${esc(i.source_url || '#')}"><code>${esc(i.function_name)}</code></a><br><small>${esc(i.qualified_name)}</small></td><td><span class="badge ${t}">${esc(i.function_type)}</span></td><td><code>${esc(i.module)}</code></td><td>${chips(i)}</td><td><span class="badge review">${esc(i.recommended_action)}</span><br><span class="badge ${p}">${esc(i.priority)}</span></td><td class="num">${esc(i.called_by_count)}</td><td class="num">${esc(i.calls_count)}</td><td><a class="source-link" href="${esc(i.source_url || '#')}">${esc(i.source_path || 'Source unavailable')}</a></td></tr>${open?`<tr class="details"><td colspan="9"><div class="details-panel"><p class="signal-reason"><strong>Why flagged:</strong> ${esc(signalReason(i))}</p><div class="details-grid"><section><h3>Callers</h3>${linkedList(i.callers)}</section><section><h3>Callees</h3>${linkedList(i.callees)}</section><section><h3>Direct internal helpers</h3>${linkedList(i.direct_internal_helpers || [])}</section><section><h3>Metrics</h3><p><strong>Depth:</strong> ${esc(i.deepest_call_chain_depth ?? '—')}</p><p><strong>Repeated helpers:</strong> ${esc(i.repeated_helper_count ?? 0)}</p></section></div></div></td></tr>`:''}`; }).join(''); }
...
document.addEventListener('click', e=>{ const sort=e.target.closest('[data-sort]'); if(sort){ const k=sort.dataset.sort; state.sortDir=state.sortKey===k?state.sortDir*-1:1; state.sortKey=k; update(); } const toggle=e.target.closest('[data-toggle]'); if(toggle){ const id=toggle.dataset.toggle; state.expanded.has(id)?state.expanded.delete(id):state.expanded.add(id); renderTable(); } const quick=e.target.closest('[data-quick]'); if(quick){ state.quick=quick.dataset.quick; state.activeCard='all'; update(); } const card=e.target.closest('[data-card]'); if(card) applyCard(card.dataset.card); });
