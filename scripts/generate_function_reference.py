...
def _render_refactor_dashboard_html(flow_data: dict[str, Any]) -> str:
    ...
    embedded_flow_data = json.dumps(flow_data, sort_keys=True, separators=(",", ":"))
...
def _render_runtime_inventory_source_html(flow_data: dict[str, Any]) -> str:
    """Render the source runtime inventory shell used by the combined dashboard."""
    del flow_data
    return """<!doctype html><html><body><main>
<section id="runtimeExportToolbar" class="export-toolbar" aria-label="Advanced cleanup and export actions">
<h2>Export runtime inventory cleanup packet</h2>
<div class="export-mode-toggle"><label><input id="quickExportMode" name="exportMode" value="quick" type="radio" checked>Quick export</label><label><input id="manualExportMode" name="exportMode" value="manual" type="radio">Manual selection</label></div>
<p><button id="selectVisible" type="button">Export all visible runtime assets</button><button id="clearSelected" type="button">Export manually selected rows</button></p>
<p id="selectedCount">Quick export visible rows</p>
</section>
<section class="scope-banner"><div class="scope-banner-row"><div><strong id="scopeBannerName">All runtime assets</strong><p id="scopeBannerHelp">Showing all runtime assets.</p><span id="currentFunctionScope" class="scope-current-function" hidden></span></div><button id="showAllRuntimeAssets" type="button">Show all runtime assets</button></div></section>
<div id="inventoryStatusBar" class="inventory-status-bar">Selected <span id="selectedStatusCount">0</span> Showing <span id="showingStatusCount">0</span> Total <span id="totalStatusCount">0</span> Public callables <span id="publicCallableStatusCount">0</span></div>
<p class="inventory-status-help">Showing is affected by the current architecture scope, search text, and table column filters.</p>
<div class="inventory-filter-grid"><label>Search runtime inventory <input id="searchBox"></label><button id="resetFilters" type="button">Clear search</button></div>
<p id="resultCount"></p>
<div class="callable-review-table-wrap"><table class="callable-review-table" data-table-controls="excel"><tbody id="inventoryBody"></tbody></table></div>
