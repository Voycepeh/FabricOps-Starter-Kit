...
def callable_docs_link(
    ...
    *,
    ...
    if context == "reference":
        return f"../../api/reference/{symbol_name}/"
    if context == "notebook":
        return f"../../api/reference/{symbol_name}/"
    raise RuntimeError(f"Unknown link context: {context}")
...
def main() -> None:
    ...
    for generated_module_page in MODULE_DIR.glob("*.md"):
        generated_module_page.unlink()
    ...
    for qn, node in sorted(node_by_qn.items()):
        ...
        if node["exported"]:
            ...
            ] or ['<span class="reference-chip">Usage detection may exclude indirect references</span>']
            page_chip_lines = [
                '<p class="reference-catalogue-item-meta reference-catalogue-item-badges">',
            ...
            lines = [
                ...
                *page_chip_lines,
                ...
                + (
                    ...
                    else "Starter notebook usage is generated from detected notebook references and may exclude indirect or generated usage."
                ),
                "",
                "## Source",
                "",
                f'<a class="reference-source-link" href="{source_ref}">View source on GitHub</a>',
                "",
                f"Implemented in `{source_path}`" + (f":{source_start_line}" if source_start_line else "") + ".",
                "",
