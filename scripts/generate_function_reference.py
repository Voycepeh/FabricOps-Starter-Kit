...
def _build_callable_flow_data(
    ...
    *,
    ...
    unique_inventory_identity_count = len(set(inventory_identities))
    summary_counts["inventory_row_count"] = len(function_inventory)
    summary_counts["unique_inventory_identity_count"] = unique_inventory_identity_count
    summary_counts["duplicate_inventory_identity_count"] = len(function_inventory) - unique_inventory_identity_count
...
def _runtime_inventory_section_html(flow_data: dict[str, Any]) -> str:
    ...
    return (
        ...
        'It only describes runtime code assets under <code>src/fabricops_kit</code> that support public callables or template runtime references. Unreachable does not mean safe to delete. It means no public/template runtime path was found by the scanner.</small></p>'
        f"{main_html}"
        "</section>"
    )
