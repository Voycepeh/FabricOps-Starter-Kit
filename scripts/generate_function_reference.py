...
def _render_combined_refactor_dashboard_html(flow_data: dict[str, Any]) -> str:
    ...
    dashboard = dashboard.replace(
        ...
        "<h2>Global summary</h2><p class=\"overview-help\"><small>Choose an architecture scope, inspect the call graph when one exists, review the scoped runtime inventory, then export one AI refactor packet from selected inventory assets.</small></p><h2>Architecture scope table</h2><p class=\"overview-help\"><small>Scope rows include every public callable flow plus All runtime assets and Others / Unreachable runtime assets.</small></p><div class=\"toolbar-row architecture-scope-specials\"><button id=\"scopeAllRuntimeAssets\" type=\"button\">All runtime assets</button><button id=\"scopeUnreachableRuntimeAssets\" type=\"button\">Others / Unreachable runtime assets</button></div>",
    )
    dashboard = dashboard.replace(
        '<section class="export-toolbar" aria-label="Advanced cleanup and export actions">',
        '<section class="export-toolbar" aria-label="Advanced cleanup and export actions" hidden>',
        1,
    )
