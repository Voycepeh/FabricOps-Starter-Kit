...
def _render_dashboard_architecture_scope_rows(flow_data: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        '<tr data-architecture-scope-special="unreachable" tabindex="0"><td><button type="button" class="callable-button">Others / Cannot trace back to a public callable</button></td><td><span class="badge warn">Verify possible orphan</span></td><td class="num">—</td>'
        f'<td class="num">{unreachable_count}</td><td class="num">—</td><td></td></tr>',
    ]
    if not public_flows:
