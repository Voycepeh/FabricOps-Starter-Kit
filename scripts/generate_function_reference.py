...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .kpi-group { margin: 1rem 0 1.25rem; } .kpi-group h2 { font-size: .95rem; margin: 0 0 .55rem; color:#334155; text-transform:uppercase; letter-spacing:.04em; } .cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); gap: .75rem; margin: 0; }
    .card { border: 1px solid var(--border); border-left: .25rem solid var(--blue); border-radius: .9rem; padding: .8rem .9rem; background: var(--soft); text-align: left; cursor: pointer; min-height: 6.25rem; }
    ...
    .card strong { display:block; font-size:1.6rem; line-height:1; } .card span { color:#1f2937; font-weight:700; display:block; margin-top:.35rem; } .card small { display:block; margin-top:.35rem; line-height:1.25; }
...
function renderCards() { const groups=[['Cleanup opportunities',[['thin','Inline candidates',summary.thin_wrapper_candidates,'info','Small helpers likely safe to flatten'],['merge','Merge candidates',summary.merge_candidates,'info','Adjacent helpers worth consolidating'],['abstraction','Review abstraction',summary.review_abstraction_candidates,'medium','Check if single-use boundary still helps']]],['Hygiene signals',[['Unreachable','Unreachable',summary.unreachable_or_unclassified_functions,'medium','Not reachable from public API graph'],['Review','Review manually',summary.manual_review_candidates,'medium','Needs maintainer judgment'],['High','High / medium review',Number(summary.high_priority_candidates||0)+Number(summary.medium_priority_candidates||0),'high','Actionable hygiene queue, not broad reuse warnings']]],['Inventory and stability',[['all','Total functions',summary.total_functions_discovered,'info','All discovered functions'],['Public','Public API entrypoints',summary.public_api_entrypoints,'info','Notebook-facing package exports'],['Internal','Reachable internal helpers',summary.reachable_internal_helpers,'protect','Internal helpers used by public APIs'],['Protect','Protect helpers',summary.protect_helpers,'protect','Stable dependency. Avoid casual edits']]]]; $('summaryCards').innerHTML=groups.map(([title,cards])=>`<section class="kpi-group"><h2>${esc(title)}</h2><div class="cards">${cards.map(([key,label,value,klass,help])=>`<button type="button" class="card ${klass} ${state.activeCard===key?'active':''}" data-card="${key}" title="${esc(help)}"><strong>${esc(value ?? 0)}</strong><span>${esc(label)}</span><small>${esc(help)}</small></button>`).join('')}</div></section>`).join(''); }
