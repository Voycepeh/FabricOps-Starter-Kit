...
REFACTOR_SIGNAL_RECOMMENDATIONS = {
    ...
    "Single-use internal helper": "Inline candidate",
...
ACTION_LEGEND = {
    ...
    "Inline candidate": "Helper used by only one function; inline only when that improves readability without changing behavior.",
...
def _build_function_inventory(
    ...
    for qn in sorted(node_by_qn, key=lambda item: (node_by_qn[item]["module_name"], node_by_qn[item]["callable_name"].lower(), item)):
        ...
        if architecture_signals:
            ...
            recommended_action = "Shared internal helper" if len(inbound) > 1 else refactor.get("recommended_action", "Next step")
            priority = refactor.get("priority", "Low")
...
DECISION_RECOMMENDATIONS = {
    ...
    "architecture_violation": "Fix architecture violation",
    "merge": "Inspect merge candidates",
...
DISPLAY_LABEL_MAP = {
    ...
    "deep_chain": "Long chain",
    "large_downstream_surface": "Wide dependency",
    "single_use_helper_candidate": "Merge candidate",
    "inline_candidate": "Merge candidate",
    "review_abstraction_value": "Inline candidate",
}
...
def _decision_warnings(flow: dict[str, Any], callee_rows: list[dict[str, Any]]) -> list[str]:
    ...
    if flow["single_use_helper_candidate_count"]:
        warnings.add("Merge candidate found")
...
def _build_callable_flow_data(
    ...
    *,
    ...
    summary_counts["public_api_surface"] = {
        ...
        "long_call_chains": sum(1 for flow in public_entrypoint_flow if "Long chain" in flow["warnings"]),
