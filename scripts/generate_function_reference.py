...
REFACTOR_SIGNAL_RECOMMENDATIONS = {
    "Review for merge": "Review for merge",
...
ACTION_LEGEND = {
    ...
    "Review for merge": "This helper is used by one function, and that function uses it once. Review whether keeping it separate makes the code easier to read.",
...
def _refactor_priority(signals: list[str]) -> str:
    ...
    if "Heavily used helper" in signals:
        return "Protect"
    if "Review for merge" in signals:
        return "High"
...
def _build_refactor_inventory(
    ...
    for qn in sorted_qns(set(node_by_qn)):
        ...
        if inbound_count == 1 and call_site_count == 1 and not recursive:
            signals.append("Review for merge")
    ...
    summary_counts = {
        "review_for_merge_helpers": sum(1 for row in inventory if "Review for merge" in row["signals"]),
...
DECISION_RECOMMENDATIONS = {
    ...
    "merge": "Inspect helpers marked Review for merge",
...
DISPLAY_LABEL_MAP = {
    ...
    "Single-use helper candidate": "Review for merge",
    ...
    "large_downstream_surface": "Many dependencies",
    "single_use_helper_candidate": "Review for merge",
    "inline_candidate": "Review for merge",
...
def _decision_warnings(flow: dict[str, Any], callee_rows: list[dict[str, Any]]) -> list[str]:
    ...
    if flow["downstream_callable_count"] >= LARGE_DEPENDENCY_SURFACE_THRESHOLD:
        warnings.add("Many dependencies")
