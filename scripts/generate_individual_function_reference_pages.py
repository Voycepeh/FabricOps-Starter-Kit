...
METADATA_FIELD_DESCRIPTIONS = {
    ...
    "processing_json": "Serialized editable processing definition owned by the Data Contract version.",
    "processing_source": "How draft processing was initialized: Catalogue, previous contract, default, or manual.",
    "contract_payload_json": "Serialized immutable contract payload populated when the version is frozen.",
    ...
    "access_id": "Stable identifier for one observed SQL permission row.",
    ...
    "frequency_json": "Serialized frequency distribution for the column when enabled.",
    ...
    "severity": "Severity recorded for a Guardrail result.",
    ...
    "guardrail_result_id": "Stable identifier for the aggregate Guardrail result row.",
    "execution_type": "Whether the Guardrail outcome came from enforce or validate contract execution.",
    "result_id": "Stable identifier for the runtime result payload.",
    "reason": "Human-readable reason recorded for the Guardrail outcome.",
    ...
    "result_payload_json": "Serialized aggregate result payload written for the Guardrail outcome.",
...
METADATA_RELATED_FUNCTIONS = {
    ...
    "METADATA_DATA_CONTRACT": ["widget_data_contract"],
    "METADATA_DATA_CATALOGUE": ["profile_table", "widget_data_contract"],
...
def _expanded_downstream_qualified_names(
    root_qn: str,
    relationships: list[dict[str, Any]],
) -> list[str]:
    """Reconstruct cycle-safe downstream path rows from normalized direct relationships."""
...
def _load_public_call_flow_inventory(path: Path = PUBLIC_CALL_FLOW_DATA_PATH) -> dict[str, Any]:
    ...
    for record in public_functions:
        ...
        record["downstream_callable_count"] = (
            len(_expanded_downstream_qualified_names(str(qualified_name), relationships))
            if qualified_name
            else 0
        )
    return data
    ...
    *,
    ...
    if context == "release":
        lines.extend([
            ...
            "",
            f"- Downstream callables: {record.get('downstream_callable_count', 0)}",
...
ROLE_TAGS_BY_NAME = {
    ...
    "read_lakehouse_json": ["public_api_entrypoint", "fabric_io_entrypoint"],
...
def _metadata_index_key_html(fields: list[str]) -> str:
