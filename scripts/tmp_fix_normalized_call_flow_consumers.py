...
# 3. Reference generator derives presentation-only expanded downstream rows from normalized edges.
...
new_loader = '''def _expanded_downstream_qualified_names(
    root_qn: str,
...
old_catalogue = '        downstream_callables = [row["qualified_name"] for row in public_flow.get("flow", [])[1:] if row.get("qualified_name")]\n'
new_catalogue = '''        downstream_callables = _expanded_downstream_qualified_names(
            str(public_flow["qualified_name"]),
            dashboard_inventory_data.get("relationships", []),
        )
'''
if old_catalogue in text:
    text = text.replace(old_catalogue, new_catalogue, 1)
elif new_catalogue not in text:
    raise SystemExit("catalogue downstream flow read not found")
text = text.replace("explanatory page for the v2 public-function call-flow architecture contract", "explanatory page for the v3 normalized public-function call-flow architecture contract")
text = text.replace(": v2 architecture contract generated by", ": v3 normalized architecture contract generated by")
