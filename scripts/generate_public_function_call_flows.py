...
function treeNode(root,node){const typeClass=`tree-type-${String(node.function_type||'helper').replaceAll('_','-')}`;return `<div class="tree-node" style="margin-left:${Math.min((node.depth||0)*.85,4.25)}rem"><div class="tree-row-main"><span>▸</span><a class="tree-function" href="${githubSourceUrl(node)}" target="_blank" rel="noopener">${esc(node.function_name)}(...)</a><span class="badge ${esc(typeClass)}">${esc(typeLabel(node.function_type))}</span><span class="tree-file">${esc(basename(node.source_path))}</span>${(node.derived_violation_types||[]).map(x=>`<span class="badge danger">${esc(x)}</span>`).join(' ')}</div></div>`}
