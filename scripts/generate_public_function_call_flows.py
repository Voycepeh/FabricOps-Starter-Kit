...
function download(name,text,type){const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([text],{type})); a.download=name; a.click()} function exportPacket(){download('fabricops-public-flow-refactor-packet-v2.json',JSON.stringify(packet(),null,2),'application/json');$('exportStatus').textContent='Downloaded Codex/GPT-ready cleanup packet.'} function exportOrphanPacket(){download('fabricops-unused-cleanup-packet-v1.json',JSON.stringify(orphanPacket(),null,2),'application/json');$('orphanExportStatus').textContent='Downloaded unused cleanup packet.'}
