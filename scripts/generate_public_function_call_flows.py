...
function exportScope(){return $('exportScope')?$('exportScope').value:'full_selected_flow'} function exportScopeReason(scope){return ({full_selected_flow:'Includes all functions in the selected public callable flow.',checked_functions_only:'Includes only rows manually checked in the selected callable inventory.',inline_candidates_only:'Includes only deterministic inline candidates.',architecture_violations_only:'Includes only functions with Type 1–Type 6 architecture violations.',promote_to_shared_candidates_only:'Includes only promote-to-shared candidate functions from this flow.'})[scope]} function exportScopeSlug(scope){return ({full_selected_flow:'full-flow',checked_functions_only:'selected-functions',inline_candidates_only:'inline-candidates',architecture_violations_only:'architecture-violations',promote_to_shared_candidates_only:'promote-shared'})[scope]||String(scope||'custom-scope').replace(/_/g,'-')} function cleanupModeSlug(mode){return mode==='preserve_compatibility'?'compatible':'breaking'} function getFunctionsForExportScope({flow,exportScope:scope}){const inv=uniqInventory(flow||[]); if(scope==='checked_functions_only')return inv.filter(n=>selectedInventory.has(n.qualified_name)); if(scope==='inline_candidates_only')return inv.filter(n=>!!n.inline_candidate); if(scope==='architecture_violations_only')return inv.filter(n=>(n.derived_violation_types||[]).length); if(scope==='promote_to_shared_candidates_only')return inv.filter(n=>!!n.promote_to_shared_candidate); return inv} function scopedInventory(inv){return getFunctionsForExportScope({flow:inv,exportScope:exportScope(),cleanupMode:$('cleanupMode')?$('cleanupMode').value:'breaking_cleanup'})} function exportScopeHelpText(scope){return ({full_selected_flow:'All functions in this selected flow will be included.',checked_functions_only:'Only checked rows from this flow will be included.',inline_candidates_only:'Only deterministic inline candidate functions from this flow will be included.',architecture_violations_only:'Only functions with Type 1–Type 6 architecture violations from this flow will be included.',promote_to_shared_candidates_only:'Only promote-to-shared candidate functions from this flow will be included.'})[scope]} function updateExportScopeHelp(){if(!$('exportScopeHelp')||!selectedPublic)return; const scope=exportScope(), exportedFunctions=getFunctionsForExportScope({flow:selectedPublic.flow,exportScope:scope,cleanupMode:$('cleanupMode').value}); $('exportScopeHelp').textContent=exportScopeHelpText(scope); $('exportFunctionCount').textContent=`${exportedFunctions.length} functions will be exported`} function buildCleanupPacketFilename({cleanupMode,exportScope,functionCount}){const timestamp=new Date().toISOString().slice(0,16).replace(/[-:]/g,'').replace('T','-');return `fabricops-codex-cleanup_${cleanupModeSlug(cleanupMode)}_${exportScopeSlug(exportScope)}_${functionCount}-functions_${timestamp}.json`}
...
function download(name,text,type){const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([text],{type})); a.download=name; a.click()} function exportPacket(){const payload=packet(), evidence=payload.evidence_packet||{}, filename=buildCleanupPacketFilename({cleanupMode:evidence.cleanup_mode,exportScope:evidence.export_scope,functionCount:evidence.exported_function_count});download(filename,JSON.stringify(payload,null,2),'application/json');$('exportStatus').textContent='Downloaded Codex/GPT-ready cleanup packet.'} function exportOrphanPacket(){download('fabricops-unused-cleanup-packet-v1.json',JSON.stringify(orphanPacket(),null,2),'application/json');$('orphanExportStatus').textContent='Downloaded unused cleanup packet.'}
