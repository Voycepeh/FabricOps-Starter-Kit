...
<section id="architectureWorkflow" class="workflow-panel active"><section class="architecture-summary-cards"><article class="architecture-summary-card info"><div class="card-kicker">Main review</div><div class="card-title">Public functions</div><strong id="card-public">0</strong></article><article class="architecture-summary-card risk"><div class="card-kicker">Main review</div><div class="card-title">Public functions with architecture violation</div><strong id="card-warnings">0</strong></article><article class="architecture-summary-card review"><div class="card-kicker">Main review</div><div class="card-title">Public functions with large width/depth</div><strong id="card-large">0</strong></article><article class="architecture-summary-card good"><div class="card-kicker">Supported by</div><div class="card-title">Shared helper functions</div><strong id="card-shared-helpers">0</strong></article><article class="architecture-summary-card info"><div class="card-kicker">Supported by</div><div class="card-title">Nested private functions</div><strong id="card-private-functions">0</strong></article></section>
...
<details class="overview-help"><summary>Signal rules</summary>Public rows show only Large width/depth and Architecture violation. See the full signal rules and calculations panel above.</details>
...
function githubSourceUrl(r){return `https://github.com/Voycepeh/FabricOps-Starter-Kit/blob/main/${esc(r.source_path)}#L${esc(r.source_start_line||1)}`} function functionLink(r){return `<a class="source-link" href="${githubSourceUrl(r)}" target="_blank" rel="noopener"><code>${esc(r.function_name)}</code></a>`} function hasPublicSignal(f,signal){const publicSignals=f.public_signals||[]; if(signal==='large_width_or_depth')return !!f.has_large_width_or_depth||publicSignals.includes(signal)||(Number(f.width??f.direct_call_count??0)>10)||(Number(f.depth??f.max_depth??0)>5); if(signal==='architecture_violation')return !!f.has_architecture_violation||publicSignals.includes(signal)||(Number(f.architecture_violation_count??0)>0)||((f.flow||[]).some(n=>(n.violation_types||[]).length)); return false} function publicSignalsFor(f){const out=[]; if(hasPublicSignal(f,'large_width_or_depth'))out.push('large_width_or_depth'); if(hasPublicSignal(f,'architecture_violation'))out.push('architecture_violation'); return out} function badges(a){const filtered=(a||[]).filter(x=>x==='large_width_or_depth'||x==='architecture_violation');return filtered.length?filtered.map(x=>`<span class="badge ${x==='architecture_violation'?'danger':'warn'}">${esc(x==='large_width_or_depth'?'Large width/depth':'Architecture violation')}</span>`).join(' '):'<span class="badge muted">None</span>'}
...
function filteredPublic(){const q=$('searchBox').value.toLowerCase(), sig=$('signalFilter').value; return DATA.public_functions.filter(f=>(!q||[f.function_name,f.qualified_name,f.source_path,publicSignalsFor(f).join(' ')].join(' ').toLowerCase().includes(q))&&(!sig||publicSignalsFor(f).includes(sig))).sort(compareRows(sortKey,sortDir))}
function renderPublicTable(){publicRows=filteredPublic(); $('rowCount').textContent=`${publicRows.length} of ${DATA.public_functions.length} public functions`; $('public-table').innerHTML=publicRows.map(f=>`<tr data-public-flow-row="${esc(f.qualified_name)}" class="${selectedPublic&&selectedPublic.qualified_name===f.qualified_name?'active':''}" tabindex="0"><td>${functionLink(f)}</td><td>${esc(f.source_path)}</td><td>${esc(f.width??f.direct_call_count)}</td><td>${esc(f.scope??f.transitive_function_count)}</td><td>${esc(f.depth??f.max_depth)}</td><td>${esc((f.files_touched||[]).length)}</td><td>${badges(publicSignalsFor(f))}</td></tr>`).join('')}
function selectPublic(qn){selectedPublic=DATA.public_functions.find(f=>f.qualified_name===qn)||DATA.public_functions[0]; selectedInventory.clear(); uniqInventory(selectedPublic.flow).forEach(n=>selectedInventory.add(n.qualified_name)); renderPublicTable(); renderSelected(); updateCounts()}
