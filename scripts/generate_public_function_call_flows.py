...
function treeSummaryChips(node){const stats=hiddenTreeStats(node);if(!stats.downstream)return '';const chips=[`<span class="badge muted">children ${stats.children}</span>`,`<span class="badge muted">downstream ${stats.downstream}</span>`,`<span class="badge muted">max depth ${stats.maxDepth}</span>`];if(stats.violations)chips.push(`<span class="badge danger">violations ${stats.violations}</span>`);chips.push(`<span class="badge muted">+ ${stats.downstream} hidden</span>`);return `<span class="tree-summary">${chips.join(' ')}</span>`}
function treeNode(root,node){const typeClass=`tree-type-${String(node.function_type||'helper').replaceAll('_','-')}`, children=(node._children||[]), expanded=expandedTreeNodes.has(node._tree_key);const toggle=children.length?`<button class="tree-toggle" type="button" data-tree-node-toggle="${esc(node._tree_key)}" aria-label="${expanded?'Collapse':'Expand'} ${esc(node.function_name)} downstream calls" aria-expanded="${expanded?'true':'false'}">${expanded?'▾':'▸'}</button>`:'<span class="tree-toggle-placeholder">•</span>';return `<div class="tree-node" style="margin-left:${Math.min((node.depth||0)*.85,4.25)}rem"><div class="tree-row-main">${toggle}<a class="tree-function" href="${githubSourceUrl(node)}" target="_blank" rel="noopener">${esc(node.function_name)}(...)</a><span class="badge ${esc(typeClass)}">${esc(typeLabel(node.function_type))}</span><span class="tree-file">${esc(basename(node.source_path))}</span>${(node.derived_violation_types||[]).map(x=>`<span class="badge danger">${esc(x)}</span>`).join(' ')}${children.length&&!expanded?treeSummaryChips(node):''}</div></div>`}
function setTreeDepth(depth){if(!selectedPublic)return;expandedTreeNodes.clear();const rows=buildTreeRows(selectedPublic.flow);if(depth==='all')rows.filter(n=>(n._children||[]).length).forEach(n=>expandedTreeNodes.add(n._tree_key));else rows.filter(n=>(n._children||[]).length&&Number(n.depth||0)<depth).forEach(n=>expandedTreeNodes.add(n._tree_key));renderSelected()}
...
function initializeTreeExpansion(flow){expandedTreeNodes.clear();buildTreeRows(flow).filter(n=>(n._children||[]).length&&Number(n.depth||0)<2).forEach(n=>expandedTreeNodes.add(n._tree_key))}
...
function renderSelected(){const f=selectedPublic;if(!f)return; $('selectedSummary').innerHTML=`<div class="flow-summary-card"><h3>${functionLink(f)}</h3><p><code>${esc(f.qualified_name)}</code> · <span>${esc(f.source_path)}</span></p><div class="flow-meta"><div><strong>${esc(f.derived_width)}</strong><br>Width</div><div><strong>${esc(f.derived_scope)}</strong><br>Scope</div><div><strong>${esc(f.derived_depth)}</strong><br>Depth</div><div><strong>${esc((f.files_touched||[]).length)}</strong><br>Files</div></div><p>${badges(publicSignalsForFunction(f))}</p></div>`; $('selected-call-tree').innerHTML=visibleTreeRows(f.flow||[]).map(n=>treeNode(f,n)).join(''); const allRows=uniqInventory(f.flow); refreshInventoryTypeFilter(allRows); inventoryRows=filteredInventoryRows(allRows); $('inventoryRowCount').textContent=`${inventoryRows.length} of ${allRows.length} rows`; const checkEnabled=exportScope()==='checked_functions_only'; $('inventoryBody').innerHTML=inventoryRows.map(n=>{const included=scopedInventory(allRows).some(x=>x.qualified_name===n.qualified_name);return `<tr class="${included?'row-selected':''}"><td><input class="inventory-check" type="checkbox" data-qn="${esc(n.qualified_name)}" ${selectedInventory.has(n.qualified_name)?'checked':''} ${checkEnabled?'':'disabled'}><span class="scope-state muted">${checkEnabled?'Manual':included?(exportScope()==='full_selected_flow'?'Included by full flow':'Included'):'Excluded by scope'}</span></td><td>${functionLink(n)}</td><td>${esc(n.function_type)}</td><td>${esc(n.source_path)}</td><td>${esc(n.depth)}</td><td>${esc(n.distinct_parent_qualified_names.join(', ')||'—')}</td><td>${violationBadges(n)}</td><td>${esc((n.derived_violation_details||[]).join(' | ')||'—')}</td><td>${n.inline_candidate?'Yes':'No'}</td><td>${n.promote_to_shared_candidate?'Yes':'No'}</td></tr>`}).join(''); updateExportScopeHelp()}
