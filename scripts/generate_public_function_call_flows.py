...
function githubSourceUrl(r){return `https://github.com/Voycepeh/FabricOps-Starter-Kit/blob/main/${esc(r.source_path)}#L${esc(r.source_start_line||1)}`} function functionLink(r){return `<a class="source-link" href="${githubSourceUrl(r)}" target="_blank" rel="noopener"><code>${esc(r.function_name)}</code></a>`} function publicSignalsForFunction(f){const signals=new Set(f.public_signals||[]); if(f.has_large_width_or_depth)signals.add('large_width_or_depth'); if(f.has_architecture_violation)signals.add('architecture_violation'); return [...signals].filter(signal=>signal==='large_width_or_depth'||signal==='architecture_violation')} function hasPublicSignal(f,signal){return publicSignalsForFunction(f).includes(signal)} function signalLabel(signal){return signal==='large_width_or_depth'?'Large width/depth':signal==='architecture_violation'?'Architecture violation':signal} function badges(a){const filtered=(a||[]).filter(x=>x==='large_width_or_depth'||x==='architecture_violation');return filtered.length?filtered.map(x=>`<span class="badge ${x==='architecture_violation'?'danger':'warn'}">${esc(signalLabel(x))}</span>`).join(' '):'<span class="badge muted">None</span>'}
...
function renderPublicTable(){publicRows=filteredPublic(); $('rowCount').textContent=`${publicRows.length} of ${DATA.public_functions.length} public functions`; $('public-table').innerHTML=publicRows.map(f=>`<tr data-public-flow-row="${esc(f.qualified_name)}" class="${selectedPublic&&selectedPublic.qualified_name===f.qualified_name?'active':''}" tabindex="0"><td>${functionLink(f)}</td><td>${esc(f.source_path)}</td><td>${esc(f.width??f.direct_call_count)}</td><td>${esc(f.scope??f.transitive_function_count)}</td><td>${esc(f.depth??f.max_depth)}</td><td>${esc((f.files_touched||[]).length)}</td><td>${badges(publicSignalsForFunction(f))}</td></tr>`).join('')}
