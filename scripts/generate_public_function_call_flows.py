...
function depthButtonValues(maxDepth){if(maxDepth<=0)return [];if(maxDepth<=6)return Array.from({length:maxDepth},(_,i)=>i+1);return [1,2,3,4,5,maxDepth]}
function renderTreeDepthControls(f){const maxDepth=selectedMaxCallDepth(f.flow), depthButtons=depthButtonValues(maxDepth).map(d=>`<button type="button" data-tree-depth="${d}">Expand ${d===maxDepth&&maxDepth>6?'max depth':`depth ${d}`}</button>`).join('');$('treeDepthControls').innerHTML=`${depthButtons}<button type="button" data-tree-depth="all">Expand all</button><button type="button" data-tree-depth="0">Collapse all</button><a class="section-jump source-link" href="#selected-callable-inventory-heading">Go to selected callable inventory</a>`}
