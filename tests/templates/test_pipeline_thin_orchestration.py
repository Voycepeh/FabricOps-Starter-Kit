...
def test_pipeline_agreement_selector_registers_notebook_context():
    ...
    start_block = code[code.index("AGREEMENT_CONTEXT = widget_select_agreement(") : code.index("# SOURCE AREA") if "# SOURCE AREA" in code else code.index("source_table = ")]
    assert 'notebook_type="02_pipeline"' in start_block
    assert "register_notebook=True" in start_block
    assert "select_agreement=True" not in start_block
    assert "AGREEMENT = AGREEMENT_CONTEXT.agreement" in start_block
