...
def test_widget_updates_dataframe_state_without_rendering_views():
    """The widget owns selection and state only, not DataFrame rendering."""
    import inspect
    import fabricops_kit.widgets.shared as shared

    source = inspect.getsource(public_widget)
    assert "display(state[" not in source
    assert "state.update(views)" in source
    assert "Rerun the notebook display cells" in source
