...
def test_public_call_flow_json_includes_release_lifecycle_contract_fields() -> None:
    """Validate release manifests enrich public call-flow JSON with lifecycle data."""
    payload = flows.build_payload()

    profile = next(item for item in payload["public_functions"] if item["function_name"] == "profile_dataframe")
    run_guardrails = next(item for item in payload["public_functions"] if item["function_name"] == "run_table_guardrails")

    assert payload["metadata"]["lifecycle_source"] == "docs/releases/manifests/*.yml"
    assert "0.1.0" in payload["metadata"]["release_versions"]
    assert profile["lifecycle_status"] == "live"
    assert profile["live_since"] == "0.1.0"
    assert profile["contract_classification"] == "live_public_function"
    assert profile["contract_display"] == "Live · Live since 0.1.0"
    assert profile["live_critical_dependency_count"] > 0
    assert {row["notebook"] for row in profile["notebook_dependents"]} >= {"02_pipeline", "99_explore"}
    assert {row["notebook"] for row in profile["live_notebook_dependents"]} == {"99_explore"}
    assert run_guardrails["lifecycle_status"] == "preview"
    assert run_guardrails["contract_display"].startswith("Preview")


def test_public_call_flow_json_tracks_live_critical_internal_helpers() -> None:
    """Validate helpers beneath Live public functions are marked Live-critical."""
    payload = flows.build_payload()
    read_table = next(item for item in payload["public_functions"] if item["function_name"] == "read_lakehouse_table")

    helper = next(item for item in read_table["flow"] if item["qualified_name"] != read_table["qualified_name"])
    assert read_table["qualified_name"] in helper["live_dependents"]
    assert helper["contract_classification"] == "live_critical_internal"
    assert helper["contract_display"] == "Live-critical internal"


def test_public_call_flow_json_tracks_live_metadata_contract_impact() -> None:
    """Validate functions can be Preview while supporting Live metadata contracts."""
    payload = flows.build_payload()
    setup = next(item for item in payload["public_functions"] if item["function_name"] == "setup_metadata_tables")
    relationships = {row["table_name"]: row for row in setup["transitive_metadata_contract_relationships"]}

    assert setup["lifecycle_status"] == "preview"
    assert setup["supports_live_metadata_contracts"] is True
    assert setup["writes_live_metadata"] is True
    assert setup["contract_classification"] == "preview_function_supporting_live_contracts"
    assert setup["contract_display"] == "Preview · Supports Live contracts"
    assert relationships["METADATA_DATA_AGREEMENT"]["is_live_metadata_contract"] is True
    assert relationships["METADATA_DATA_CATALOGUE"]["contract"] == "METADATA_DATA_CATALOGUE schema v1"
