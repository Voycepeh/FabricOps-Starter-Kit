...
def test_write_warehouse_reference_page_renders_docstring_intro_and_notes(tmp_path, monkeypatch) -> None:
    ...
    assert "passes the repartitioned DataFrame into the Warehouse write connector" in normalized_warehouse_page
    assert "executes the requested connector write mode" in normalized_warehouse_page
    assert "unique run-scoped Warehouse staging table" not in normalized_warehouse_page
    assert "transactional T-SQL target mutation" not in normalized_warehouse_page
...
def test_public_callable_usage_notes_are_family_standardized() -> None:
    ...
    widget_text = (API_REFERENCE_DIR / "widget_data_contract.md").read_text(encoding="utf-8")
...
def test_concept_pages_link_back_to_key_callable_references() -> None:
    ...
    assert "[`pipeline_read`](../../api/reference/pipeline_read.md)" in lineage_table
