...
def test_callable_flow_page_and_json_cover_public_surface() -> None:
    ...
    assert '<strong class="reference-kpi-value">325</strong>' in reference_text
    assert '<strong class="reference-kpi-value">26</strong>' in reference_text
    assert '<strong class="reference-kpi-value">69</strong>' in reference_text
    ...
    assert summary_counts["callable_kind"]["function"] == 95
...
def test_every_callable_page_has_curated_public_reference_sections() -> None:
    ...
    for page in callable_pages:
        ...
        assert "Module: <code>" not in text, page
        assert "## Source" in text, page
        assert "View source on GitHub" in text, page
...
def test_public_callable_call_tree_renders_before_description() -> None:
    ...
    assert title_index < description_index < usage_index
...
def test_module_pages_are_not_generated() -> None:
    """Verify module-first public reference pages are not generated."""
    module_dir = ROOT / "docs" / "api" / "modules"

    assert not any(module_dir.glob("*.md"))
