...
def test_function_catalogue_uses_callable_internal_filters() -> None:
    ...
    assert 'data-function-type-filter="internal"> Internal' in page
    assert "Public functions intended for notebook authors." in page
    assert "Supporting functions used by the package. Shown for transparency and debugging." in page
...
def test_internal_functions_are_indexed_for_filtering() -> None:
    page = _reference_index()

    assert 'data-function-type="callable"' in page
    assert 'data-function-type="internal"' in page
    assert 'class="reference-chip reference-chip-type reference-chip-internal">Internal</span>' in page
