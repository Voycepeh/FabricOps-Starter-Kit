...
def _target_scope(source_preps: list[dict[str, Any]]) -> dict[str, Any]:
    """Validate source participation and return the target's complete-input scope."""
    if not source_preps:
...
def write_pipeline_prep(
    ...
    *,
    ...
    prepared_scope = _target_scope(source_preps)
    ...
    if strategy == "overwrite" and partition_column:
        ...
        prepared_scope = {
            ...
            "scope": {"type": "partition", "column": partition_column, "values": partition_values},
        }
