...
def profile_and_register_table(
    ...
    *,
    ...
    processing_scope : mapping, optional
        Runtime scope returned by ``read_pipeline_prep()["scope"]``. A source
        ``incremental_subset`` scope returns a diagnostic profile without
        writing canonical metadata. Full-dataset and omitted scopes retain
        canonical registration behaviour.
    register_profile : bool, default=True
        Whether a full source profile represents the registered physical table
        and should become canonical metadata. Set to ``False`` for an
        engineer-authored query result that does not represent the complete
        registered table. Incremental subsets are diagnostic regardless of
        this value.

    Returns
    -------
    pyspark.sql.DataFrame
        For canonical registration, a Spark DataFrame containing the rows
        appended to ``METADATA_DATA_PROFILED``. For an incremental subset or
        ``register_profile=False``, the non-persisted diagnostic profile
        returned by ``profile_dataframe``.

    Raises
    ------
    ValueError
        If the role, target, configured store, schema, table identity,
        processing scope, or registration choice is invalid.
    RuntimeError
    ...
    * ``METADATA_DATA_CATALOGUE``: updated or newly added table and column
    ...
    if not isinstance(register_profile, bool):
        raise ValueError("register_profile must be Boolean.")
    ...
    if scope is not None and scope["type"] == "skip":
        raise ValueError("A source resolved to skip must not be profiled.")
    diagnostic = (scope is not None and scope["type"] != "full_dataset") or not register_profile
    if diagnostic:
        if normalized_profile_role != "source":
            raise ValueError("Diagnostic processing scopes and register_profile=False are valid only for sources.")
