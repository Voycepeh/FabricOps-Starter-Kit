...
def widget_render_data_steward(*, spark: Any, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return _render_data_steward_widget(spark=spark, config=config, env=env)
...
def _create_or_update_data_steward(*, spark: Any, config: Any, env: str, values: dict[str, Any], custom_fields: dict[str, Any] | None = None, committed_by: str | None = None, committed_at: str | None = None, runtime_context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    row["is_active"] = False if explicit_active not in (None, "") and not _to_bool(explicit_active) else bool(_active_steward({**row, "is_active": row.get("is_active", "")}, config))
...
def _render_data_steward_widget(*, spark: Any, config: Any, env: str, display_widget: bool = True) -> dict[str, Any]:
    ...
    existing_rows = [row for row in _list_data_stewards(config, env, spark_session=spark, active_only=False, missing_ok=True) if _row_id(row)]
    ...
    form = {field: _standard_widget(field, options=steward_role_options if field == "steward_role" else None) for field in fields}
    custom = _render_custom_fields(widget_config)

    def _refresh_existing_options(selected_id: str | None = None) -> None:
        rows = [row for row in _list_data_stewards(config, env, spark_session=spark, active_only=False, missing_ok=True) if _row_id(row)]
    ...
    def _save(_: Any) -> None:
        ...
        with output:
            try:
                values = {key: _to_iso_date(widget.value) if key in {"effective_from", "effective_to"} else widget.value for key, widget in form.items()}
