...
from fabricops_kit.config.metadata_keys import _build_dq_rule_key, build_metadata_column_key, build_metadata_table_key
...
def _approved_column_identity(profile_row: dict[str, Any], review_row: dict[str, Any], *, env: str | None = None) -> dict[str, str]:
    ...
    table_key = str(
        ...
        or build_metadata_table_key(store_type, layer, schema_name, table)
    )
    return {
        "metadata_column_key": str(_value(profile_row, "metadata_column_key") or review_row.get("metadata_column_key") or build_metadata_column_key(table_key, col)),
...
def _base_guardrail_rule_record(state: Mapping[str, Any], *, guardrail_type: str, rule_type: str, column_name: str = "", parameters: Mapping[str, Any] | None = None, severity: str = "warning", description: str = "", policy: Mapping[str, Any] | None = None, bypass_reason: str = "", actor: str | None = None, action: str = "submit", source_notebook_type: str = "02_pipeline", created_by_role: str = "engineering", config: Any = None) -> dict[str, Any]:
    ...
    table_key = str(state.get("metadata_table_key") or build_metadata_table_key(
...
def _catalogue_physical_identity(row: dict[str, Any]) -> dict[str, str]:
    ...
    if not table_key:
        table_key = build_metadata_table_key(asset_kind or "lakehouse", schema_or_layer, _value(row, "schema_name", None), table)
    return {
...
def _evaluate_governance_readiness(
    ...
    *,
    ...
    table_key = str(_value(first_profile, "metadata_table_key") or selection.get("metadata_table_key") or build_metadata_table_key(
