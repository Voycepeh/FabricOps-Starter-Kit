...
MODULE_DOCS_METADATA: list[ModuleDocMetadata] = [
    ...
    {"module_name": "schema_contracts", "visibility": "public", "module_summary": "Owns dataset-level schema contract suggestions, review state, versioned persistence, validation, enforcement, and evidence.", "sidebar_group": "2. Analyst / data scientist", "sidebar_include": True},
...
TEMPLATE_FLOW_DOCS: list[TemplateFlowDocMetadata] = [{'notebook_key': '00_env_config',
 ...
 {'notebook_key': '01_data_agreement',
  ...
  'segment_intro': 'Form notebook that supports two agreement-intake layouts for A/B testing. Option A renders a compact section switcher through `render_agreement_intake_app(...)`. Option B renders separate widget cells for Data Steward, Data Agreement, and Agreement Evidence through `render_data_steward_widget(...)`, `render_data_agreement_widget(...)`, and `render_agreement_evidence_widget(...)`. Both layouts write append-only metadata and agreement intake selects active steward rows.',
  'segments': [{'title': 'Segment 1: Option A compact app',
                'text': 'FabricOps currently supports two `01_da` layouts. Option A is a compact section switcher via `render_agreement_intake_app(...)`. Option B is separate widget cells for Data Steward, Data Agreement, and Agreement Evidence. Use Option B if Fabric output scrolling feels jumpy or if users prefer rerunning one section at a time. Do not run both options in the same notebook execution unless testing UX, because both render widgets for the same metadata tables.',
                'symbols': ['render_agreement_intake_app']},
               {'title': 'Segment 2: Option B separate widgets',
                'text': 'Render separate Data Steward, Data Agreement, and Agreement Evidence cells with `render_data_steward_widget(...)`, `render_data_agreement_widget(...)`, and `render_agreement_evidence_widget(...)` when you want each section to be rerun independently.',
                'symbols': ['render_data_steward_widget', 'render_data_agreement_widget', 'render_agreement_evidence_widget']}]},
 {'notebook_key': '02_ex',
 ...
 {'notebook_key': '03_pc',
  ...
  'segment_intro': 'Core production pipeline flow for clean evidence creation, inline runtime audit columns, large-table write tuning, and controlled publishing. 03_pc uses build_runtime_audit_fields for metadata runtime values, then adds dataframe audit columns inline. Audit columns are always useful; hashes, datetime features, and bucket columns are specialized patterns outside the default path.',
  'segments': [{'title': 'Segment 1: Runtime setup, parameters, agreement selection, and registration',
                'symbols': ['setup_notebook', 'select_agreement', 'get_selected_agreement', 'current_notebook_active_registrations']},
               {'title': 'Segment 2: Read source data',
                'symbols': ['read_lakehouse_table', 'read_warehouse_table', 'read_lakehouse_csv', 'read_lakehouse_parquet']},
               {'title': 'Segment 3: Profile and write reusable catalogue evidence',
                'symbols': ['profile_dataframe', 'write_lakehouse_table']},
               {'title': 'Segment 4: Transform, add runtime audit columns, and publish outputs',
                'symbols': ['build_runtime_audit_fields', 'write_lakehouse_table', 'write_warehouse_table']},
               {'title': 'Segment 5: Read back output, profile output, and write lineage',
                'symbols': ['read_lakehouse_table', 'read_warehouse_table', 'profile_dataframe', 'build_lineage_records', 'write_lakehouse_table']}]}]
...
PUBLIC_SYMBOL_DOCS: list[PublicSymbolDocMetadata] = [{'symbol_name': 'setup_notebook',
 ...
 {'symbol_name': 'get_selected_agreement',
  ...
  'role': 'essential',
  'summary_override': None},
 {'symbol_name': 'render_agreement_intake_app',
  'module': 'data_agreement',
  'kind': 'function',
 ...
 {'symbol_name': 'render_agreement_evidence_widget',
  'module': 'data_agreement',
  'kind': 'function',
  'template_notebook': '`01_da_<agreement>`',
 ...
 {'symbol_name': 'render_data_agreement_widget',
  'module': 'data_agreement',
  'kind': 'function',
  'template_notebook': '`01_da_<agreement>`',
  'template_segment': 'Segment 2: Option B separate widgets',
 ...
 {'symbol_name': 'render_data_steward_widget',
  'module': 'data_agreement',
  'kind': 'function',
  'template_notebook': '`01_da_<agreement>`',
  'template_segment': 'Segment 2: Option B separate widgets',
 ...
 {'symbol_name': 'run_dq_rule_review_widget',
  ...
  'template_segment': '—',
  'role': 'optional',
  'summary_override': 'Render the notebook widget for human review and approval/rejection of candidate DQ rules.'},
 ...
 {'symbol_name': 'validate_dq_rules',
  ...
  'role': 'optional',
  'summary_override': None},
 {'symbol_name': 'SchemaContractValidationError',
  'module': 'schema_contracts',
  'kind': 'class',
  'template_notebook': '—',
  'template_segment': '—',
  'role': 'optional',
  'summary_override': 'Focused exception raised when schema-contract enforcement is configured to fail.'},
 {'symbol_name': 'suggest_schema_contract',
  'module': 'schema_contracts',
  'kind': 'function',
  'template_notebook': '—',
  'template_segment': 'Exploration/schema contract setup',
  'role': 'essential',
  'summary_override': 'Draft dataset-level schema-contract column rows from profile metadata or a Spark DataFrame without writing metadata.'},
 {'symbol_name': 'review_schema_contract',
  'module': 'schema_contracts',
  'kind': 'function',
  'template_notebook': '—',
  'template_segment': 'Exploration/schema contract setup',
  'role': 'optional',
  'summary_override': 'Render a lightweight schema-contract approval widget and return editable approved settings and column rows.'},
 {'symbol_name': 'build_schema_contract_review_state',
  'module': 'schema_contracts',
  'kind': 'function',
  'template_notebook': '—',
  'template_segment': 'Exploration/schema contract setup',
  'role': 'optional',
  'summary_override': 'Normalize non-UI schema-contract review state for tests and notebooks.'},
 {'symbol_name': 'write_schema_contract',
  'module': 'schema_contracts',
  'kind': 'function',
  'template_notebook': '—',
  'template_segment': 'Exploration/schema contract setup',
  'role': 'essential',
  'summary_override': 'Persist a versioned approved source or target dataset schema contract and normalized column rows.'},
 {'symbol_name': 'load_schema_contract',
  'module': 'schema_contracts',
  'kind': 'function',
  'template_notebook': '`03_pc_<agreement>_<source>_to_<target>`',
  'template_segment': 'Schema validation',
  'role': 'essential',
  'summary_override': 'Load the latest approved schema contract for one specific source or target dataset.'},
 {'symbol_name': 'validate_schema',
  ...
  'template_segment': 'Schema validation',
  'role': 'essential',
  'summary_override': 'Validate a Spark DataFrame shape against approved dataset-level schema contract columns without writing metadata or stopping the pipeline.'},
 {'symbol_name': 'enforce_schema_result',
  'module': 'schema_contracts',
  'kind': 'function',
  'template_notebook': '`03_pc_<agreement>_<source>_to_<target>`',
  'template_segment': 'Schema validation',
  'role': 'essential',
  'summary_override': 'Apply observe, warn, or fail behavior to a schema validation result.'},
 {'symbol_name': 'build_schema_validation_evidence',
  'module': 'schema_contracts',
  'kind': 'function',
  'template_notebook': '`03_pc_<agreement>_<source>_to_<target>`',
  'template_segment': 'Schema validation',
  'role': 'essential',
  'summary_override': 'Build structured schema-drift evidence rows for metadata storage.'},
 {'symbol_name': 'write_schema_validation_evidence',
  'module': 'schema_contracts',
  'kind': 'function',
  'template_notebook': '`03_pc_<agreement>_<source>_to_<target>`',
  'template_segment': 'Schema validation',
  'role': 'essential',
  'summary_override': 'Append schema validation evidence through the configured metadata lakehouse route.'},
