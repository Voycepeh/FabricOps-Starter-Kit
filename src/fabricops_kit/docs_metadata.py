...
TEMPLATE_FLOW_DOCS: list[TemplateFlowDocMetadata] = [{'notebook_key': '00_env_config',
 ...
 {'notebook_key': '01_data_agreement',
  ...
  'segment_intro': 'Form notebook that renders the agreement-intake app through one framework helper. The helper renders a tabbed app for Data Steward maintenance, Data Agreement maintenance, and optional Agreement Evidence upload. Both tables are append-only, and agreement intake selects active steward rows.',
  'segments': [{'title': 'Segment 1: Render the agreement-intake app',
                'text': 'Use `render_agreement_intake_app(...)` for the default form notebook. It renders one tab each for Data Steward, Data Agreement, and optional Agreement Evidence upload. Downstream notebooks use `select_agreement(...)` and `get_selected_agreement()` to bind work to a committed version.',
