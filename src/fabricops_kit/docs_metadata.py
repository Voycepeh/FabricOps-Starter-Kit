...
TEMPLATE_FLOW_DOCS: list[TemplateFlowDocMetadata] = [{'notebook_key': '00_env_config',
 ...
 {'notebook_key': '01_data_agreement',
  ...
  'segment_intro': 'Form notebook that renders the agreement-intake app through one framework helper. The helper uses a section switcher so only one workflow section is visible at a time in Fabric notebooks for Data Steward maintenance, Data Agreement maintenance, and optional Agreement Evidence upload. Both tables are append-only, and agreement intake selects active steward rows.',
  'segments': [{'title': 'Segment 1: Render the agreement-intake app',
                'text': 'Use `render_agreement_intake_app(...)` for the default form notebook. The `01_da` intake app uses a section switcher so only one workflow section is visible at a time in Fabric notebooks. Downstream notebooks use `select_agreement(...)` and `get_selected_agreement()` to bind work to a committed version.',
